...
# --- TEMPLATES SPECS ---
TEMPLATES = {
    "T1": { "name": "1. แนวตั้ง 1+2 (3 รูป)", "specs": [(960, 1920), (1920, 1920), (1920, 1920)] },
    "T2": { "name": "2. แนวตั้ง 1+3 (4 รูป)", "specs": [(1280, 1920), (1920, 1920), (1920, 1920), (1920, 1920)] },
    "T3": { "name": "3. แนวตั้ง 2+3 (5 รูป)", "specs": [(1920, 1920), (1920, 1920), (1920, 1280), (1920, 1280), (1920, 1280)] },
    "T4": { "name": "4. แนวนอน 1+2 (3 รูป)", "specs": [(1920, 960), (1920, 1920), (1920, 1920)] },
    "T5": { "name": "5. แนวนอน 1+3 (4 รูป)", "specs": [(1920, 1280), (1920, 1920), (1920, 1920), (1920, 1920)] },
    "T6": { "name": "6. จัตุรัส 4 (4 รูป)", "specs": [(1920, 1920), (1920, 1920), (1920, 1920), (1920, 1920)] },
    "T7": { "name": "7.1 เดี่ยว-จัตุรัส", "specs": [(1200, 1200)] },
    "T8": { "name": "7.2 เดี่ยว-แนวตั้ง", "specs": [(1080, 1350)] },
    "T9": { "name": "7.3 เดี่ยว-แนวนอน", "specs": [(1200, 630)] }
}

class AutoGridV59:
    def __init__(self, root):
        self.root = root
        self.root.title("AutoGrid V59 (STOP Button Added)")
        ...
        self.server_port = 8098 # New port
        self.httpd = None
        self.deploy_path = ""
        self.running = False # Flag for STOP button
    ...
    def setup_main_tab(self):
        ...
        for k in sorted(TEMPLATES.keys()):
            ...
            self.chk_vars[k] = v
            tk.Checkbutton(f_temp, text=k, variable=v, bg="#1e1e1e", fg="white", selectcolor="#333", command=self.update_calc).grid(row=row_c, column=col_c, sticky="w", padx=5)
            col_c += 1
            if col_c > 3:
                col_c = 0
                row_c += 1
    ...
    def setup_logs_tab(self):
        ...
        self.log(">> System Ready... (V59 with STOP Button)")
    ...
    def generate_web(self, output_root, user, password, content_map, start_date_str):
        ...
        # HTML - ADDED PAGINATION AND CALENDAR NAVIGATION
        ...
        .grid-wrap {{ display: grid; gap: 2px; background: white; padding: 2px; border-radius: 8px; overflow: hidden; margin: 0 auto; }}
        .grid-single {{ background: white; padding: 2px; border-radius: 8px; overflow: hidden; }}
        img {{ width: 100%; height: 100%; object-fit: cover; display: block; }}

        /* T1-T6 Layouts */
        .layout-T1 {{ aspect-ratio: 1/1; grid-template-columns: 1fr 1fr; grid-template-rows: 1fr 1fr; }} .layout-T1 img:nth-child(1) {{ grid-row: 1 / 3; grid-column: 1; }} .layout-T1 img:nth-child(2) {{ grid-row: 1; grid-column: 2; }} .layout-T1 img:nth-child(3) {{ grid-row: 2; grid-column: 2; }}
        .layout-T2 {{ aspect-ratio: 1/1; grid-template-columns: 2fr 1fr; grid-template-rows: 1fr 1fr 1fr; }} .layout-T2 img:nth-child(1) {{ grid-row: 1 / 4; grid-column: 1; }} .layout-T2 img:nth-child(2) {{ grid-row: 1; grid-column: 2; }} .layout-T2 img:nth-child(3) {{ grid-row: 2; grid-column: 2; }} .layout-T2 img:nth-child(4) {{ grid-row: 3; grid-column: 2; }}
        .layout-T3 {{ aspect-ratio: 1.38/1; grid-template-columns: repeat(6, 1fr); grid-template-rows: 1.5fr 1fr; }} .layout-T3 img:nth-child(1) {{ grid-column: 1 / 4; grid-row: 1; }} .layout-T3 img:nth-child(2) {{ grid-column: 4 / 7; grid-row: 1; }} .layout-T3 img:nth-child(3) {{ grid-column: 1 / 3; grid-row: 2; }} .layout-T3 img:nth-child(4) {{ grid-column: 3 / 5; grid-row: 2; }} .layout-T3 img:nth-child(5) {{ grid-column: 5 / 7; grid-row: 2; }}
        .layout-T4 {{ aspect-ratio: 1/1; grid-template-columns: 1fr 1fr; grid-template-rows: 1fr 1fr; }} .layout-T4 img:nth-child(1) {{ grid-column: 1 / 3; grid-row: 1; }} .layout-T4 img:nth-child(2) {{ grid-column: 1; grid-row: 2; }} .layout-T4 img:nth-child(3) {{ grid-column: 2; grid-row: 2; }}
        .layout-T5 {{ aspect-ratio: 1/1; grid-template-columns: 1fr 1fr 1fr; grid-template-rows: 2fr 1fr; }} .layout-T5 img:nth-child(1) {{ grid-column: 1 / 4; grid-row: 1; }} .layout-T5 img:nth-child(2) {{ grid-column: 1; grid-row: 2; }} .layout-T5 img:nth-child(3) {{ grid-column: 2; grid-row: 2; }} .layout-T5 img:nth-child(4) {{ grid-column: 3; grid-row: 2; }}
        .layout-T6 {{ aspect-ratio: 1/1; grid-template-columns: 1fr 1fr; grid-template-rows: 1fr 1fr; }}
        ...
        function renderItem(d, platform){{
            ...
            if(platform=='fb'){{
                if(k.includes('T7')||k.includes('T8')||k.includes('T9')) h=`<div class="layout-Single">${{d.images.map(s=>`<img src="${{s}}">`).join('')}}</div>`; 
                else {{
                    let tKey = 'T6';
                    if(k.includes('T1')) tKey='T1'; else if(k.includes('T2')) tKey='T2'; else if(k.includes('T3')) tKey='T3';
                    else if(k.includes('T4')) tKey='T4'; else if(k.includes('T5')) tKey='T5'; else if(k.includes('T6')) tKey='T6';
                    h=`<div class="grid-wrap layout-${{tKey}}">${{d.images.map(s=>`<img src="${{s}}">`).join('')}}</div>`;
                }}
            }} else {{ h=`<div class="flex overflow-x-auto gap-2">${{d.images.map(s=>`<img src="${{s}}" class="w-32 aspect-[4/5] rounded">`).join('')}}</div>` }}
                ...
                for p in ['fb', 'ig', 'tt']:
                    ...
                    for idx, path in enumerate(sorted_src):
                        with Image.open(path) as img:
                            ...
                            fn = f"{idx+1}.jpg"
                            fin.save(os.path.join(dpath, fn), quality=95)
