...
# ==============================================================================
# 1. HTML TEMPLATE (V85.2 - Final Logic)
# ==============================================================================
...
<body>
    ...
    <script>
        ...
        async function openDepositFlow() { 
            ...
            setDepType('liquor'); 
            document.getElementById('pre-gen-code').innerText = "รอสร้าง...";
            
            show('modal-deposit'); 
            // V85: NO PRE-GEN on Client, wait for server
        }
        ...
        async function submitDeposit() { 
            ...
            if(isPreviewMode()){ setTimeout(()=>{show('modal-deposit-success');document.getElementById('success-code').innerText="10199";btn.innerText=oldText;btn.disabled=false},1000);return} 
            
            // V85: Use FormData for reliability
            const formData = new FormData();
            formData.append('image', file); // Raw file
            formData.append('staff_uid', profile.userId); // SECURE: Send UID
            ...
            try { 
                ...
                Swal.fire("Error", "Save Failed: " + e.message, "error"); 
            } finally { 
...
# ==============================================================================
# 2. WORKER CODE (V85.2 - Final Logic)
# ==============================================================================
WORKER_RAW = r"""
export default {
    async fetch(request, env, ctx) {
      ...
      try {
          ...
          if (path === "/api/deposit" && request.method === "POST") {
              ...
              // LOGIC: Use 4 digits from ID + 1 random digit (Collision safe up to 10,000 active items)
              const id = res.id;
              const suffix = Math.floor(Math.random() * 10); // 1 digit (0-9)
              const idPart = (id % 10000).toString().padStart(4, '0'); // Last 4 digits of ID
              const code = idPart + String(suffix); // Total 5 digits
          ...
          if (path === "/api/claim" && request.method === "POST") {
              ...
              // Add 'COLLISION SAFE' check: claim the oldest one if code duplicates (very rare now)
          ...
          if (path === "/api/withdraw" && request.method === "POST") {
              ...
              // UPDATE DB FIRST (Priority)
              await env.DB.prepare("UPDATE deposits SET status = 'withdrawn' WHERE id = ?").bind(item.id).run();
              
              // DELETE IMAGE (Secondary - don't fail transaction if this fails)
              if (item.image_key) {
...
# ==============================================================================
# 3. PYTHON UI CONTROLLER (V85.2 - Show Pass & D1 Field)
# ==============================================================================
class AppGenerator(ctk.CTk):
    def __init__(self):
        super().__init__()
        self.title("Nightlife System V85.2 (Final)")
    ...
    def setup_ui(self):
        ...
        for k in ["Shop Name", "LIFF ID", "Worker URL", "Manager Password", "R2 Public URL", "Channel Access Token", "D1 Database ID", "Hold Table Time"]:
            ...
            # NOTE: Password visible as requested
            self.entries[k] = e
    ...
    def build(self):
        ...
        # NOTE: V85.1 SAFE MODE - NO DROP TABLE
        ...
        messagebox.showinfo("Success", "Generated V85.2! Password is now visible.")
