    ...
    async fetch(request, env, ctx) {
      ...
      try {
          ...
          if (path === "/api/login" && request.method === "POST") {
              ...
              if (body.pass === "8888") return new Response(JSON.stringify({ success: true }), { headers: corsHeaders });
          ...
          if (path === "/api/withdraw" && request.method === "POST") {
              ...
              const note = body.remark ? ` [Note: ${body.remark}]` : "";
              await addLog('withdraw', staff.name, `Withdrew: ${item.item_name} (Code: ${body.code})${note}`, item.image_key);
              return new Response(JSON.stringify({ success: true }), { headers: corsHeaders });
          }
      ...
      return new Response("API Ready v85.5", { headers: corsHeaders });
    },
