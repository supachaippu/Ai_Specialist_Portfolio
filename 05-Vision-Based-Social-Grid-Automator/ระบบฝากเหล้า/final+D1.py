...
# ==============================================================================
# 1. HTML TEMPLATE (V84 - TEXT FIXED)
# ==============================================================================
...
<head>
    ...
    <style>
        ...
        .code-display-box { background: rgba(255,255,255,0.05); border: 1px solid var(--accent); border-radius: 16px; padding: 15px; text-align: center; margin-bottom: 15px; animation: fadeIn 0.5s ease; width: 100%; box-sizing: border-box; }
...
<body>
    <div class="main-container">
        ...
        <div id="view-customer" class="hidden animate-fade-in">
             ...
             <div id="menu-container" class="menu-layout w-full pb-4">
                <div onclick="show('view-booking')" class="btn-menu menu-item-1"><span class="icon">📅</span><span class="label" data-i18n="menu_booking">จองโต๊ะ</span><span class="sub-label" data-i18n="menu_booking_sub">สำรองที่นั่งล่วงหน้า</span></div>
        ...
        <div id="modal-deposit" class="fixed inset-0 z-[70] hidden flex-col bg-black p-4">
            ...
            <div class="code-display-box"><div class="code-label">เขียนรหัสนี้ที่ข้างขวด</div><div id="pre-gen-code" class="code-value">...</div></div>
    ...
    <script>
        ...
        let staffPollCount = 0; const staffPollMax = 3;  
        ...
        async function openDepositFlow() { 
            ...
            try {
                if(isPreviewMode()){ preGeneratedCode = "10589"; } 
                else {
            ...
            } catch(e) { document.getElementById('pre-gen-code').innerText = "ERROR"; }
        }
        ...
        async function submitDeposit() { 
            ...
            try { 
                const res = await fetch(CFG.worker + "/api/deposit", { 
                    method: 'POST', body: JSON.stringify({ staff_name: sessionStorage.getItem('staffName') || 'Staff', item_name: brand, item_type: depType, amount: amt, remarks: remark, image: base64Img, pre_code: preGeneratedCode }) 
                }); 
                const data = await res.json(); 
        ...
        async function manualClaimInput() { 
            const { value: code } = await Swal.fire({ 
                ...
                text: 'ดูเลข 5 หลักจากหน้าจอพนักงาน', // FIXED TEXT
                input: 'text', 
        ...
        // V82: SHARE GIFT (TRANSFER) FEATURE
            ...
            try { 
                ...
                data.forEach(item => { 
                    ...
                    if (item.status === 'active') { 
                        ...
                        // Construct absolute Image URL for Flex Message
...
export default {
    async fetch(request, env, ctx) {
      ...
      try {
          ...
          if (path === "/api/next-code") {
              ...
              if (lastItem && lastItem.deposit_code && lastItem.deposit_code.length === 5) {
                  ...
                  if (!isNaN(lastSeq)) { nextSeq = lastSeq + 1; if (nextSeq > 999) nextSeq = 1; }
              }
          ...
          if (path === "/api/deposit" && request.method === "POST") {
              const body = await request.json();
              const imgString = body.image.split(',')[1]; const imgBuffer = Uint8Array.from(atob(imgString), c => c.charCodeAt(0)); 
              const filename = `dep_${Date.now()}.jpg`; await env.BUCKET.put(filename, imgBuffer);
      ...
      return new Response("API Ready v82", { headers: corsHeaders });
    },
...
class AppGenerator(ctk.CTk):
    def __init__(self):
        super().__init__()
        self.title("Nightlife System V84 (Ultimate Text Fixed)")
    ...
    def setup_ui(self):
        ...
        self.entries = {}
        # V82: REMOVED "D1 Database ID"
        for k in ["Shop Name", "LIFF ID", "Worker URL", "Manager Password", "R2 Public URL", "Channel Access Token", "Hold Table Time"]:
        ...
        self.phone.pack_propagate(False) # Stop phone from shrinking
    ...
    def build(self):
        ...
        # V82: Removed Check, Auto-fill Default
        db_id = "YOUR_D1_ID_HERE"
