...
# ==============================================================================
# 1. HTML TEMPLATE (V85.6 - Same as V85.5)
# ==============================================================================
...
<body>
    ...
    <script>
        ...
        let currentDepCode = ""; // V85.5: Store current pre-gen code
        ...
        async function openDepositFlow() { 
            ...
            genNewCode(); // Generate immediately
            show('modal-deposit'); 
        }
        ...
        async function submitDeposit() { 
            ...
            formData.append('deposit_code', currentDepCode); // V85.5: Send pre-gen code
...
# ==============================================================================
# 2. WORKER CODE (V85.6 - Added 14 Days Notification)
# ==============================================================================
WORKER_RAW = r"""
export default {
    async fetch(request, env, ctx) {
      ...
      try {
          ...
          if (path === "/api/deposit" && request.method === "POST") {
              ...
              const depositCode = formData.get('deposit_code'); // V85.5: Receive Pre-Gen Code
              ...
              // V85.5: Insert with specific code
          ...
          if (path === "/api/claim" && request.method === "POST") {
              ...
              // Add 'COLLISION SAFE' check: claim the oldest one if code duplicates (ORDER BY id ASC)
          ...
          if (path === "/api/withdraw" && request.method === "POST") {
              ...
              // FIX: Handle collisions by picking the *Oldest* Active one (FIFO) if multiple exist
      ...
      return new Response("API Ready v85.6", { headers: corsHeaders });
    },
    async scheduled(event, env, ctx) {
      // V85.6: Added 14 days notification logic
...
# ==============================================================================
# 3. PYTHON UI CONTROLLER (V85.6 - Final Optimized)
# ==============================================================================
class AppGenerator(ctk.CTk):
    def __init__(self):
        super().__init__()
        self.title("Nightlife System V85.6 (Final Optimized)")
    ...
    def build(self):
        ...
        # FIX: Changed crons to "0 3 * * *" (10:00 AM BKK)
        ...
        messagebox.showinfo("Success", "Generated V85.6 (Added 14 Days Noti)!")
