...
const CONFIG = {
  ...
  ALLOWED_ORIGIN: "https://v86-crm-test-web.pages.dev",
...
function getCors(origin) {
  ...
  const ok = allowed.some(o => origin.startsWith(o)) || origin.endsWith(".pages.dev") || origin === "";
  return {
    "Access-Control-Allow-Origin": ok ? origin : CONFIG.ALLOWED_ORIGIN,
...
function rewardFlex(name, points, imgUrl) {
  return {
    type: "flex", altText: `ของรางวัล: ${name}`,
    contents: {
      type: "bubble",
      hero: { type: "image", url: imgUrl || "https://v86-crm-test-web.pages.dev/app_icon.png", size: "full", aspectRatio: "20:13", aspectMode: "cover" },
      body: {
        type: "box", layout: "vertical", contents: [
          { type: "text", text: name, weight: "bold", size: "xl" },
          {
            type: "box", layout: "baseline", margin: "md", contents: [
              { type: "text", text: "แลกด้วยแต้มเพียง", size: "sm", color: "#999", flex: 0 },
              { type: "text", text: `${points} PTS`, size: "sm", color: "#f5a623", weight: "bold", margin: "md" }
            ]
          }
        ]
      },
      footer: {
        type: "box", layout: "vertical", contents: [
          { type: "button", style: "link", height: "sm", action: { type: "uri", label: "ดูของรางวัล", uri: "https://v86-crm-test-web.pages.dev" } }
        ]
      }
    }
  };
}
// === REQUEST COUNTER (Batching to save D1 limits) ===
...
export default {
  async fetch(request, env, ctx) {
    ...
    try {
      ...
      if (p === "/api/my-coupons") {
        const uid = url.searchParams.get("uid");
        const { results } = await env.DB.prepare("SELECT * FROM redemptions WHERE uid=? ORDER BY created_at DESC").bind(uid).all();
        return json(results, cors);
      }

      // Claim coupon (mark as used by customer)
      if (p === "/api/claim" && request.method === "POST") {
        const { coupon_id, uid } = await request.json();
        await env.DB.prepare("UPDATE redemptions SET status='pending_use' WHERE id=? AND uid=?").bind(coupon_id, uid).run();
        return json({ success: true }, cors);
      }
      ...
      if (p === "/api/staff-search") {
        ...
        return json({ user, deposits: mappedDeposits }, cors);
      }
      ...
      if (p === "/api/admin/use-coupon" && request.method === "POST") {
        ...
        await env.DB.prepare("UPDATE redemptions SET status='used' WHERE id=?").bind(coupon_id).run();
        return json({ success: true }, cors);
      }
      ...
      if (p === "/api/broadcast" && request.method === "POST") {
        let target, message, format, minPoints, reward, staff_uid, image;
        ...
        if (format === "flex") {
          if (reward) {
            messages.push(rewardFlex(reward.name, reward.points || 0, reward.img));
          } else {
            messages.push({ type: "text", text: message || "📢 แจ้งจากทางร้านค่ะ" });
          }
        } else if (format === "image_text") {
          const imgUrl = customImageUrl || reward?.img || "https://v86-crm-test-web.pages.dev/app_icon.png";
          messages.push({ type: "image", originalContentUrl: imgUrl, previewImageUrl: imgUrl });
          messages.push({ type: "text", text: message || (reward ? `แนะนำของรางวัล: ${reward.name}` : "📢 แจ้งจากทางร้านค่ะ") });
        } else {
          messages.push({ type: "text", text: message || "📢 แจ้งจากทางร้านค่ะ" });
        }
        const bc = await broadcastLine(botToken, uids, messages);
        await notifyAdmins(env, `\n📢 Broadcast: ${target}\nจำนวน: ${uids.length} คน\nสำเร็จ: ${bc.success}`);
        return json({ success: bc.success, count: uids.length, detail: bc.detail }, cors);
      }
