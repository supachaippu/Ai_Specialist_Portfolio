...
export default {
  async fetch(request, env, ctx) {
    ...
    try {
      ...
      // Get dynamic price overrides for a date
      if (p === "/api/prices") {
        const date = url.searchParams.get("date");
        if (!date) return json({ overrides: null }, cors);
        const row = await env.DB.prepare("SELECT overrides FROM daily_prices WHERE date=?").bind(date).first();
        return json({ overrides: row ? JSON.parse(row.overrides) : null }, cors);
      }
      ...
      // Admin: Get daily prices (color-based overrides)
      if (p === "/api/admin/prices" && request.method !== "POST") {
        const date = url.searchParams.get("date");
        if (!date) return json({ success: true, prices: {} }, cors);
        const row = await env.DB.prepare("SELECT prices FROM daily_prices WHERE date=?").bind(date).first();
        return json({ success: true, prices: row ? JSON.parse(row.prices) : {} }, cors);
      }

      // Admin: Save daily prices (color-based overrides)
      if (p === "/api/admin/prices" && request.method === "POST") {
        const { date, prices, staff_uid } = await request.json();
        if (!await verifyRole(staff_uid, env, ["manager"])) return json({ success: false, error: "Manager Only" }, cors, 403);
        if (!date || !prices) return json({ success: false, error: "Missing date or prices" }, cors, 400);
        await env.DB.prepare("INSERT INTO daily_prices (date, prices) VALUES (?, ?) ON CONFLICT(date) DO UPDATE SET prices=excluded.prices")
          .bind(date, JSON.stringify(prices)).run();
        return json({ success: true }, cors);
      }
