...
# ==========================================
# 🧠 2. GENERATE ALL (V.82 - Staff Logic Inside)
# ==========================================
def generate_all_files():
    try:
        ...
        # 3. เตรียมโฟลเดอร์
        folder = f"web_{shop_name}_V82_StaffInside"
        ...
        <div id="staff-section">
            ...
            <a href="staff.html" class="btn-menu" style="background: linear-gradient(135deg, #059669, #064e3b);">
                <span class="icon">👮</span>
                <span class="label">เข้าสู่ระบบพนักงาน</span>
                <span class="sub-label">จัดการฝาก/เบิก (Staff Only)</span>
            </a>
        </div>
        ...
        # 5. STAFF HTML (Logic Moved Here - Gatekeeper)
        staff_template = """<!DOCTYPE html><html lang="th"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>__SHOP_NAME__ Staff</title>__COMMON_HEAD__<script src="https://unpkg.com/html5-qrcode"></script><script src="https://static.line-scdn.net/liff/edge/2/sdk.js"></script><style>.zone-btn{transition:0.2s;}.zone-btn:active{transform:scale(0.95);}</style></head><body class="p-4 bg-gray-100">
        
        <div id="access-loading" class="text-center mt-10"><div class="animate-spin h-10 w-10 border-4 border-blue-500 border-t-transparent rounded-full mx-auto"></div><p class="mt-2 text-gray-500">ตรวจสอบสิทธิ์...</p></div>
        
        <div id="access-denied" class="max-w-md mx-auto bg-white rounded-xl shadow-md p-6 text-center hidden">
            <h1 class="text-2xl font-bold text-red-500 mb-2">⛔ ไม่มีสิทธิ์เข้าถึง</h1>
            <p class="text-gray-500 mb-6">คุณยังไม่ได้ลงทะเบียนเป็นพนักงาน</p>
            <button onclick="registerStaff()" class="w-full bg-blue-600 text-white py-3 rounded-xl font-bold mb-4">📝 ลงทะเบียนขอใช้งาน</button>
            <button onclick="document.getElementById('access-denied').classList.add('hidden'); document.getElementById('sec-manager').classList.remove('hidden');" class="text-gray-400 text-sm underline">หรือเข้าสู่ระบบผู้จัดการ (Login)</button>
        </div>

        <div id="access-pending" class="max-w-md mx-auto bg-white rounded-xl shadow-md p-6 text-center hidden">
            <h1 class="text-2xl font-bold text-yellow-500 mb-2">⏳ รอการอนุมัติ</h1>
            <p class="text-gray-500">คำขอของคุณถูกส่งแล้ว<br>กรุณารอผู้จัดการอนุมัติ</p>
        </div>

        <div id="main-app" class="hidden">
            <div class="max-w-md mx-auto bg-white rounded-xl shadow-md mb-4"><div class="p-4 bg-gray-800 text-white text-center"><h1 class="text-xl font-bold">👮 Staff & Manager</h1><p id="staff-info" class="text-xs text-gray-300 mt-1">Staff Access</p><div class="flex mt-4 gap-1"><button onclick="tab('deposit')" id="btn-deposit" class="flex-1 py-2 rounded font-bold tab-active">ฝาก</button><button onclick="tab('withdraw')" id="btn-withdraw" class="flex-1 py-2 rounded font-bold tab-inactive">เบิก</button><button onclick="tab('manager')" id="btn-manager" class="flex-1 py-2 rounded font-bold tab-inactive bg-gray-600 text-white">ผจก.</button></div></div></div>
        ...
        <div id="sec-manager" class="max-w-md mx-auto hidden">
            <div class="bg-white rounded-xl shadow p-6">
                ...
                <div id="mgr-login" class="text-center space-y-3">
                    ...
                    <div class="pt-4 border-t mt-4"><p class="text-xs text-gray-400 mb-2">ถ้าขึ้น Error "no column" หรือขอสิทธิ์ไม่ได้ ให้กดปุ่มนี้</p><button onclick="fixDb()" class="bg-orange-100 text-orange-700 px-4 py-2 rounded text-sm font-bold border border-orange-200">🔧 อัปเดตฐานข้อมูล (แก้ Error)</button></div>
                </div>
                <div id="mgr-content" class="hidden">
                    ...
                    <div class="mb-4 bg-gray-50 border rounded p-3">
                        ...
                        <div id="staff-pending" class="mb-2 space-y-1"></div>
        ...
        async function initApp() {
            ...
            // Check Access Here
            checkAccess(staffProfile.userId);
        }
        initApp();
        ...
        async function checkAccess(uid) {
            try {
                ...
                if (j.role === 'staff' || j.role === 'manager') {
                    ...
                    loadRecentList();
                } else if (j.role === 'pending') {
                    document.getElementById('access-pending').classList.remove('hidden');
                } else {
                    document.getElementById('access-denied').classList.remove('hidden');
                }
            } catch(e) {
                Swal.fire('Error', 'Connect Server Error', 'error');
            }
        }
        ...
        async function registerStaff() {
            Swal.fire({
                ...
                text: `คุณ ${staffProfile.displayName} ต้องการขอสิทธิ์ใช้งาน?`,
                ...
                if (result.isConfirmed) {
                    ...
                    const r = await fetch(`${WORKER_URL}?action=request_staff`, {
                        method: 'POST', body: JSON.stringify({ uid: staffProfile.userId, name: staffProfile.displayName })
                    });
                    const j = await r.json();
                    if(j.status === 'success') {
                        location.reload(); // Reload to show pending
        ...
        async function tab(t){ 
            ...
            // Special Case: Manager Tab in Restricted Mode (Accessible only via password if not logged in main app)
            if (t==='manager' && document.getElementById('main-app').classList.contains('hidden')) {
                document.getElementById('sec-manager').classList.remove('hidden');
            }
        ...
        async function mgrLogin(){
            ...
            try {
                ...
                if(r.status===401) { Swal.fire('!','รหัสผิด (Wrong Password)','error'); } 
                    ...
                    document.getElementById('mgr-login').classList.add('hidden'); 
                    document.getElementById('mgr-content').classList.remove('hidden'); 
                    
                    // If in restricted mode (access denied page), show manager section fully
                    if(document.getElementById('access-denied').classList.contains('hidden') === false){
                       document.getElementById('access-denied').classList.add('hidden');
                       document.getElementById('sec-manager').classList.remove('hidden');
                    }
                    
                    loadMgrData(); loadStaffList(); 
                }
        ...
        async function loadStaffList(){
            try {
                ...
                if(j.pending && j.pending.length > 0) {
                    ...
                    pendingDiv.innerHTML = '<span class="text-xs text-gray-400">- ไม่มีคำขอใหม่ -</span>';
                }

                // Active List
                if(j.active && j.active.length > 0) {
                    j.active.forEach(s => {
                        const roleColor = s.role === 'manager' ? 'text-purple-600 font-bold' : 'text-gray-500';
                        listDiv.innerHTML += `<div class="flex justify-between items-center text-sm border-b py-2">
                            <span>${s.display_name} <span class="text-xs ${roleColor}">(${s.role})</span></span>
                            <button onclick="delStaff('${s.line_uid}')" class="text-red-500 text-xs border border-red-200 px-2 py-1 rounded hover:bg-red-50">ลบ</button>
                        </div>`;
                    });
                }
...
"""
        ...
        # Success Message
        messagebox.showinfo("✅ สร้างสำเร็จ (V.82 Staff Logic Inside)", f"โฟลเดอร์: {folder}\n\n1. หน้า Hub สะอาด (ไม่มี Check Status)\n2. หน้า Staff จะเช็คสิทธิ์ก่อนเข้า\n3. ถ้าไม่มีสิทธิ์ -> ปุ่มลงทะเบียนจะโผล่ที่หน้า Staff\n\n📌 อย่าลืมกด 'อัปเดตฐานข้อมูล' ใน Manager ครั้งแรกด้วยครับ")
...
# ==========================================
root = tk.Tk()
root.title("Soundabout Super App V.82 (Staff Inside)")
...
tk.Button(root, text="✨ สร้าง Web App ครบวงจร (V.82)", command=generate_all_files, bg="#16a34a", fg="white", font=("bold", 14), pady=15).pack(fill="x", padx=20, pady=20)
