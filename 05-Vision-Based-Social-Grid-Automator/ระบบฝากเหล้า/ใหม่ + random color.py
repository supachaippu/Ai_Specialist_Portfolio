...
# ==============================================================================
# 1. HTML TEMPLATE (V73 - 5 DIGIT HYBRID CODE)
# ==============================================================================
...
<head>
    ...
    <style>
        ...
        .btn-menu {
            background: linear-gradient(145deg, var(--grad-start), var(--grad-end)) !important;
            border: 1px solid rgba(255,255,255,0.15) !important;
            border-radius: __RADIUS__ !important;
            box-shadow: 0 8px 15px -3px rgba(0,0,0,0.4), inset 0 1px 0 rgba(255,255,255,0.2) !important;
        ...
        .btn-menu:active { transform: scale(0.98) !important; }
        .btn-menu:hover { box-shadow: 0 10px 20px -5px rgba(0,0,0,0.5) !important; transform: translateY(-2px); border-color: var(--accent) !important; }

        .icon { font-size: 36px !important; margin-bottom: 8px !important; display: block !important; filter: drop-shadow(0 2px 4px rgba(0,0,0,0.2)); }
        .label { font-size: 18px !important; font-weight: 600 !important; display: block !important; color: white !important; text-shadow: 0 2px 4px rgba(0,0,0,0.3); }
        .sub-label { font-size: 12px !important; color: rgba(255,255,255,0.8) !important; display: block !important; margin-top: 2px !important; font-weight: 300; }

        .staff-zone { display: none; margin-top: 20px; border-top: 1px solid rgba(255,255,255,0.1); padding-top: 15px; }
        
        .glass-input { background: rgba(0,0,0,0.2); border: 1px solid rgba(255,255,255,0.1); border-radius: 16px; padding: 12px; width: 100%; color: var(--text-main); outline: none; margin-bottom: 10px; font-size: 16px; transition: all 0.3s; box-sizing: border-box; }
        ...
        .profile-container { display: flex; flex-direction: column; align-items: center; margin-bottom: 40px; }
        ...
        .code-label { font-size: 12px; color: var(--text-sub); text-transform: uppercase; letter-spacing: 1px; }
...
<body>
    <div class="main-container">
        ...
        <div class="header-section">
            <div onclick="secretTap()" class="flex-1 text-left">
                <h1 class="text-2xl font-bold mb-0 tracking-tight" style="color: var(--text-main); text-shadow: 0 0 10px rgba(255,255,255,0.1);">__SHOP__</h1>
        ...
        <div id="view-customer" class="hidden animate-fade-in">
             <div class="profile-container">
                ...
                <h2 id="user-name" class="text-xl font-bold" style="color: var(--text-main);">Guest</h2>
                <p class="text-xs font-light" data-i18n="welcome" style="color: var(--text-sub);">ยินดีต้อนรับ กรุณาเลือกบริการ</p>
             </div>
        ...
        <div id="view-staff" class="hidden">
            <div class="staff-zone flex-1" style="display:flex !important; flex-direction: column;">
                <div style="text-align: center; margin-bottom: 15px;">
                    <span style="color:var(--accent); font-weight:bold; font-size:10px; letter-spacing:2px; text-transform:uppercase; background:rgba(255,255,255,0.05); padding:6px 12px; border-radius: 100px; border:1px solid rgba(255,255,255,0.1);">STAFF ACCESS</span>
                </div>
                <div class="flex flex-col gap-4 flex-1 justify-center">
                    <div onclick="openDepositFlow()" class="btn-menu" style="background: rgba(255,255,255,0.05) !important; border-color: rgba(255,255,255,0.1) !important; flex:1; max-height:none;">
                        <span class="icon" style="font-size: 32px !important;">📥</span><span class="label" style="font-size: 18px !important;">รับฝาก</span>
                    </div>
                    <div onclick="openWithdrawScanner()" class="btn-menu" style="background: rgba(220, 38, 38, 0.15) !important; border-color: rgba(239, 68, 68, 0.2) !important; box-shadow: none !important; flex:1; max-height:none;">
                        <span class="icon" style="font-size: 32px !important;">📤</span><span class="label" style="font-size: 18px !important; color: #fca5a5 !important;">เบิกออก</span>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div id="view-wallet" class="hidden h-full flex flex-col">
            <div class="flex justify-between mb-4 items-center shrink-0">
                ...
                <div class="flex gap-2">
                    <button id="btn-wallet-refresh" onclick="manualWalletRefresh()" class="btn-refresh-pill">
                        <span id="icon-refresh" style="font-size:14px;">↻</span> อัพเดทสถานะ
                    </button>
                </div>
            </div>
    ...
    <script>
        ...
        let allLogs = [];
        let logsLimit = 10;
        let walletPollCount = 0; 
        const walletPollMax = 2; 
        let staffPollCount = 0;  
        const staffPollMax = 3;  
        let preGeneratedCode = ""; // V73
        ...
        // V73: Open Deposit Flow with 3+2 Code Logic
        async function openDepositFlow() { 
            ...
            setDepType('liquor'); 
            document.getElementById('pre-gen-code').innerText = "Loading...";
            
            show('modal-deposit'); 
            
            try {
                if(isPreviewMode()){
                    preGeneratedCode = "10589"; // Simulation
                } else {
                    const res = await fetch(CFG.worker + "/api/next-code");
                    const data = await res.json();
                    preGeneratedCode = data.code;
                }
                document.getElementById('pre-gen-code').innerText = preGeneratedCode;
            } catch(e) {
                document.getElementById('pre-gen-code').innerText = "ERROR";
            }
        }
        ...
        async function submitDeposit() { 
            ...
            if(isPreviewMode()){ setTimeout(()=>{show('modal-deposit-success');document.getElementById('success-code').innerText=preGeneratedCode;btn.innerText=oldText;btn.disabled=false},1000);return} 
            ...
            try { 
                ...
                Swal.fire("Error", "Save Failed: " + e, "error"); 
            } finally { 
        ...
        function startStaffCheck(depositId) { 
            ...
            const check = async () => { 
                ...
                if (staffPollCount < staffPollMax) {
                    ...
                    console.log("Auto-poll stopped. Use Refresh button.");
                }
            }; 
            check(); 
        }
        ...
        function startWalletSync() { 
            ...
            walletInterval = setInterval(() => {
                if (walletPollCount < walletPollMax) {
                    loadMyWallet();
                    walletPollCount++;
                } else {
                    console.log("Wallet sync paused.");
                }
            }, 5000); 
        }
        function manualWalletRefresh() {
            walletPollCount = 0; 
            loadMyWallet(); 
            const btn = document.getElementById('icon-refresh');
...
# ==============================================================================
# 2. WORKER CODE (V73 - 3+2 CODE LOGIC)
# ==============================================================================
WORKER_RAW = r"""
export default {
    async fetch(request, env, ctx) {
      ...
      try {
          ...
          // --- V73 NEW: 3+2 CODE GENERATION LOGIC ---
          if (path === "/api/next-code") {
              // 1. Find the last running number (Storage ID)
              // Since we don't have a dedicated seq column, we parse the last deposit_code
              const lastItem = await env.DB.prepare("SELECT deposit_code FROM deposits ORDER BY id DESC LIMIT 1").first();
              
              let nextSeq = 1;
              if (lastItem && lastItem.deposit_code && lastItem.deposit_code.length === 5) {
                  const lastSeq = parseInt(lastItem.deposit_code.substring(0, 3)); // Get first 3 digits
                  if (!isNaN(lastSeq)) {
                      nextSeq = lastSeq + 1;
                      if (nextSeq > 999) nextSeq = 1; // Rollover 999 -> 001
                  }
              }
              
              // 2. Generate Random PIN (2 digits: 10-99)
              const pin = Math.floor(Math.random() * 90) + 10;
              
              // 3. Combine
              const code = String(nextSeq).padStart(3, '0') + String(pin);
              
              return new Response(JSON.stringify({ code: code }), { headers: corsHeaders });
          }
          ...
          if (path === "/api/deposit" && request.method === "POST") {
              ...
              // Use pre-code or fallback
              let code = body.pre_code;
              if (!code) {
                 // Fallback Logic (Same as above)
                 const lastItem = await env.DB.prepare("SELECT deposit_code FROM deposits ORDER BY id DESC LIMIT 1").first();
                 let nextSeq = 1;
                 if (lastItem && lastItem.deposit_code && lastItem.deposit_code.length === 5) {
                     const lastSeq = parseInt(lastItem.deposit_code.substring(0, 3));
                     if (!isNaN(lastSeq)) nextSeq = (lastSeq % 999) + 1;
                 }
                 const pin = Math.floor(Math.random() * 90) + 10;
                 code = String(nextSeq).padStart(3, '0') + String(pin);
              }
          ...
          if (path === "/api/test-push" && request.method === "POST") {
              ...
              const bubble = {
                  ...
                  "footer": {
                      "type": "box", "layout": "vertical",
                      "contents": [
                          { "type": "button", "style": "primary", "action": { "type": "uri", "label": "เปิดกระเป๋า", "uri": "https://liff.line.me/__LIFF__" } }
                      ]
                  }
              };
      ...
      return new Response("API Ready v73", { headers: corsHeaders });
    },
...
# ==============================================================================
# 3. PYTHON UI CONTROLLER (V73 - FULL LAYOUT LIBRARY)
# ==============================================================================
class AppGenerator(ctk.CTk):
    def __init__(self):
        super().__init__()
        self.title("Nightlife System Builder V73 (Hybrid 3+2 Code)")
        ...
        # --- V73: FULL 9 LAYOUT LIBRARY ---
        self.LAYOUTS = {
            "Standard (Default)": "", 
            
            "Grid (2 Columns)": """
                .menu-layout { display: grid !important; grid-template-columns: 1fr 1fr !important; gap: 15px !important; } 
                .btn-menu { display: flex !important; flex-direction: column !important; align-items: center !important; justify-content: center !important; padding: 20px !important; text-align: center !important; margin-bottom: 0 !important; height: auto !important; aspect-ratio: 1/1 !important; }
            """,
            
            "List View (Horizontal)": """
                .menu-layout { display: flex !important; flex-direction: column !important; gap: 10px !important; } 
                .btn-menu { flex-direction: row !important; align-items: center !important; justify-content: flex-start !important; padding: 15px 25px !important; text-align: left !important; height: auto !important; min-height: 80px !important; } 
                .icon { margin-bottom: 0 !important; margin-right: 20px !important; font-size: 30px !important; } 
                .label { font-size: 18px !important; } 
                .sub-label { display: none !important; }
            """,
            
            "Modern Card (Floating)": """
                .btn-menu { background: rgba(255, 255, 255, 0.05) !important; border: none !important; box-shadow: 0 20px 40px rgba(0,0,0,0.4) !important; border-radius: 24px !important; transform: translateY(0); transition: all 0.3s cubic-bezier(0.25, 0.8, 0.25, 1); } 
                .btn-menu:active { transform: scale(0.95); }
            """,
            
            "Glassmorphism (Blur)": """
                .btn-menu { background: rgba(255, 255, 255, 0.05) !important; backdrop-filter: blur(20px) !important; -webkit-backdrop-filter: blur(20px) !important; border: 1px solid rgba(255, 255, 255, 0.1) !important; box-shadow: 0 4px 30px rgba(0, 0, 0, 0.1) !important; }
            """,
            
            "Neumorphism (Soft UI)": """
                .btn-menu { background: var(--bg-color) !important; border-radius: 30px !important; box-shadow: 20px 20px 60px #0a0a0a, -20px -20px 60px #1f1f1f !important; border: none !important; } 
                .label { color: var(--text-main) !important; text-shadow: none !important; }
            """,
            
            "Cyberpunk (Neon)": """
                .btn-menu { background: #000 !important; border: 2px solid var(--accent) !important; box-shadow: 0 0 15px var(--accent), inset 0 0 15px var(--accent) !important; border-radius: 0 !important; clip-path: polygon(10% 0, 100% 0, 100% 90%, 90% 100%, 0 100%, 0 10%); } 
                .label { font-family: 'Courier New', monospace !important; letter-spacing: 2px !important; }
            """,
            
            "Minimalist (Clean)": """
                .btn-menu { background: transparent !important; border: 1px solid rgba(255,255,255,0.2) !important; box-shadow: none !important; } 
                .btn-menu:hover { background: rgba(255,255,255,0.05) !important; border-color: var(--text-main) !important; }
            """,
            
            "Compact (Small Rows)": """
                .menu-layout { gap: 8px !important; } 
                .btn-menu { padding: 15px !important; margin-bottom: 0 !important; flex-direction: row !important; justify-content: space-between !important; min-height: 60px !important; } 
                .icon { font-size: 24px !important; margin-bottom: 0 !important; } 
...
เพื่อรักษาสิทธิ์ของท่าน"""
    ...
    def setup_ui(self):
        ...
        # --- PROFILE MANAGER UI ---
        prof_frame = ctk.CTkFrame(left, fg_color="#1e293b")
        prof_frame.pack(fill="x", padx=20, pady=(20,10))
        ctk.CTkLabel(prof_frame, text="📂 Shop Profiles", font=("Arial", 14, "bold")).pack(pady=5)
        ...
        ctk.CTkButton(btn_row, text="📄 New Shop", command=self.new_profile, width=80, fg_color="#6366f1").pack(side="left", expand=True, padx=2)
        ctk.CTkButton(btn_row, text="💾 Save", command=self.save_current_profile, width=80, fg_color="#10B981").pack(side="left", expand=True, padx=2)
        ctk.CTkButton(btn_row, text="🗑️ Delete", command=self.delete_profile, width=80, fg_color="#EF4444").pack(side="left", expand=True, padx=2)

        ctk.CTkLabel(left, text="🛠️ App Config", font=("Prompt", 20, "bold")).pack(anchor="w", padx=20, pady=(10,5))
        ...
        self.create_color_btn(c_grid, "Background (Main)", 'bg_color', 0)
        self.create_color_btn(c_grid, "Grad Start", 'grad_start', 1)
        self.create_color_btn(c_grid, "Grad End", 'grad_end', 2)
        self.create_color_btn(c_grid, "Accent", 'accent', 3)
        ctk.CTkButton(self.scroll, text="🎲 Random Palette (Based on BG)", command=self.randomize_palette_based_on_bg, height=40, fg_color="#4F46E5").pack(fill="x", padx=20, pady=10)
        ...
        self.entries = {}
        for k, h in [("Shop Name", "ชื่อร้าน"), ("LIFF ID", "165xxxx-xxxx"), ("Worker URL", "Cloudflare Worker"), ("Manager Password", "รหัสผ่าน"), ("R2 Public URL", "Optional (https://pub-xxxx.r2.dev)"), ("Channel Access Token", "Line Messaging API"), ("Hold Table Time", "เวลาหลุดจอง (เช่น 20:30)")]:
        ...
        # --- BOOKING TEMPLATE ---
        ctk.CTkLabel(self.scroll, text="Booking Message Template", font=("Prompt", 14, "bold"), text_color="#EAB308").pack(anchor="w", padx=20, pady=(15,5))
        ctk.CTkLabel(self.scroll, text="Variables: {shop}, {name}, {phone}, {date}, {time}, {pax}, {hold}", font=("Arial", 10), text_color="#aaa").pack(anchor="w", padx=20)
        self.txt_booking = ctk.CTkTextbox(self.scroll, height=150)
        self.txt_booking.pack(fill="x", padx=20, pady=5)
        ...
        ctk.CTkButton(left, text="🚀 GENERATE APP", command=self.build, height=60, fg_color="#10B981", font=("Prompt", 18, "bold")).pack(fill="x", padx=20, pady=20)

        self.preview = ctk.CTkFrame(self, fg_color="#000", corner_radius=0); self.preview.grid(row=0, column=1, sticky="nsew", rowspan=3)
        ctk.CTkLabel(self.preview, text="Preview V73 (3+2 Code)", text_color="#555").pack(pady=20)
    ...
    def new_profile(self):
        ...
        self.update_preview_text(None)
        
    def save_current_profile(self):
        name = self.entries["Shop Name"].get().strip()
        if not name: return messagebox.showwarning("Warning", "Enter Shop Name to save profile")
        
        data = {k: v.get() for k,v in self.entries.items()}
        data['colors'] = self.vars.copy()
        data['layout'] = self.combo_layout.get()
        data['bk_tmpl'] = self.txt_booking.get("1.0", "end-1c") # Save template
    ...
    def load_selected_profile(self, name):
        if name in self.profiles:
            ...
            if 'colors' in data: 
                self.vars.update(data['colors'])
            if 'layout' in data:
                self.combo_layout.set(data['layout'])
    ...
    def pick_color(self, key):
        try:
            ...
            if c: 
                self.vars[key] = c
                if key == 'bg_color': self.randomize_palette_based_on_bg()
                self.update_preview_ui()
        except Exception as e: messagebox.showerror("Color Error", str(e))
    ...
    def randomize_palette_based_on_bg(self):
        ...
        if l < 0.5: 
            ...
            self.vars['text_sub'] = '#94a3b8'
            h_acc = (h + random.uniform(0.3, 0.7)) % 1.0 
            self.vars['accent'] = self.rgb2hex(*[x*255 for x in colorsys.hls_to_rgb(h_acc, 0.6, 0.9)])
            h_grad = (h + random.uniform(-0.1, 0.1)) % 1.0
            self.vars['grad_start'] = self.rgb2hex(*[x*255 for x in colorsys.hls_to_rgb(h_grad, min(1, l+0.2), s)])
            self.vars['grad_end'] = self.rgb2hex(*[x*255 for x in colorsys.hls_to_rgb(h_grad, max(0, l-0.1), s)])
        else: 
            self.vars['text_main'] = '#0f172a'
            self.vars['text_sub'] = '#475569'
            h_acc = (h + 0.5) % 1.0
            self.vars['accent'] = self.rgb2hex(*[x*255 for x in colorsys.hls_to_rgb(h_acc, 0.4, 0.8)])
            self.vars['grad_start'] = '#334155'
            self.vars['grad_end'] = '#0f172a'
    ...
    def update_preview_ui(self, event=None):
        ...
        self.p_header.configure(text_color=self.vars['text_main']); 
    ...
    def build(self):
        try:
            ...
            r2_url = self.entries["R2 Public URL"].get().strip() 
            ...
            if r2_url and not r2_url.startswith("http"): r2_url = "https://" + r2_url 
            if r2_url.endswith("/"): r2_url = r2_url[:-1]
