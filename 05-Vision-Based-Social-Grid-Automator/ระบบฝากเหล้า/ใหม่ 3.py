...
# ==============================================================================
# 1. HTML TEMPLATE (V43 - HARDCODED UI TO MATCH REFERENCE IMAGE)
# ==============================================================================
...
<head>
    ...
    <style>
        body {
            ...
            background-color: #0b1120; /* Dark Navy Background like Image */
            color: white;
            min-height: 100vh;
        ...
        /* --- 1. PROFILE CARD (MATCHING REFERENCE) --- */
        .profile-container {
        ...
        .profile-img {
            ...
            height: 50px;
            border-radius: 50%;
            border: 2px solid #fbbf24; /* Gold border like image */
            object-fit: cover;
        }
        .profile-text h2 {
            font-size: 20px;
        ...
        /* --- 2. MAIN BUTTONS (EXACT REPLICA OF IMAGE) --- */
        .menu-layout {
            display: flex;
        ...
        .btn-menu {
            /* The Exact Blue Gradient from Screenshot */
            background: linear-gradient(180deg, #2b3a88 0%, #172145 100%) !important;
            
            border: 1px solid rgba(255,255,255,0.05);
            border-radius: 24px !important; /* Visual match */
            padding: 45px 0 !important; /* Tall padding */
            width: 100%;
        ...
        .btn-menu:active {
            transform: scale(0.96); /* Motion Effect */
        }
        ...
        .sub-label {
            ...
            color: #cbd5e1 !important; /* Lighter gray */
        ...
        .glass-input:focus { border-color: #fbbf24; background: rgba(0,0,0,0.5); }
        ...
        .swal2-popup { background: #1e293b !important; color: white !important; border-radius: 20px !important; }
        .swal2-title { color: #fbbf24 !important; }
        .swal2-confirm { background-color: #fbbf24 !important; color: black !important; }
        
        /* Layout overrides for other styles (if selected in Python) */
        __LAYOUT_CSS__
    </style>
</head>
<body class="p-6 max-w-md mx-auto w-full relative">
    ...
    <div id="loading" class="fixed inset-0 z-50 flex flex-col items-center justify-center bg-black/90 backdrop-blur-sm">
        <div class="animate-spin rounded-full h-12 w-12 border-4 border-t-transparent" style="border-color: #fbbf24;"></div>
        <p class="mt-4 text-sm" style="color: #94a3b8;">Connecting...</p>
    </div>

    <div class="flex justify-between items-center mb-6">
        <div onclick="secretTap()">
            <h1 class="text-2xl font-bold select-none cursor-pointer" style="color: #fbbf24;">__SHOP__</h1>
            <p class="text-xs select-none" style="color: #94a3b8;">Nightlife System</p>
        </div>
        <div class="flex gap-2 items-center">
            ...
            <button id="btn-staff-switch" onclick="toggleStaffMode()" class="hidden text-xs bg-white/10 px-3 py-1 rounded-full border border-white/5 text-gray-300 flex items-center gap-1"><span>🔄</span> Staff</button>
        ...
        <div id="menu-container" class="menu-layout">
            <div onclick="show('view-booking')" class="btn-menu menu-item-1">
                ...
                <div class="text-wrap">
                    ...
                    <span class="sub-label" data-i18n="menu_booking_sub">Reservation</span>
                </div>
            </div>
            <div onclick="show('view-wallet'); startWalletSync();" class="btn-menu menu-item-2">
                <span class="icon">🥃</span>
                <div class="text-wrap">
                    <span class="label" data-i18n="menu_wallet">My Wallet</span>
                    <span class="sub-label" data-i18n="menu_wallet_sub">ฝากเหล้า / เช็ครายการ</span>
                </div>
            </div>
        </div>
    </div>
    ...
    <div id="view-staff" class="hidden space-y-4">
        ...
        <div class="grid grid-cols-2 gap-4">
            <div onclick="openDepositFlow()" class="btn-menu" style="background: #1f2937 !important; padding: 20px !important; margin-bottom: 0 !important; border-radius: 16px !important;">
                <span class="icon">📥</span><span class="label" style="font-size:16px !important;">รับฝาก</span>
            </div>
            <div onclick="openWithdrawScanner()" class="btn-menu" style="background: linear-gradient(135deg, #3f1010, #1a0505) !important; border-color: #ef4444 !important; padding: 20px !important; margin-bottom: 0 !important; border-radius: 16px !important;">
                <span class="icon">📤</span><span class="label text-red-400" style="font-size:16px !important;">เบิกออก</span>
            </div>
        </div>
    </div>
    ...
    <div id="view-wallet" class="hidden">
        <div class="flex justify-between mb-4">
            <button onclick="stopWalletSync(); show('view-customer')" class="text-sm flex items-center gap-1" style="color: #94a3b8;">← <span data-i18n="back">Back</span></button>
            <button onclick="manualClaimInput()" class="text-xs bg-[#fbbf24] text-black px-3 py-1 rounded-full font-bold" data-i18n="btn_manual_code">🔑 กรอกรหัส</button>
        </div>
        <h2 class="text-xl font-bold mb-4" data-i18n="wallet_title">🍾 My Collection</h2>
        <div class="text-sm font-bold text-[#fbbf24] border-b border-white/10 pb-2 mb-4" data-i18n="wallet_active">📦 รายการคงเหลือ (Active)</div>
        <div id="wallet-active" class="space-y-4 pb-4 min-h-[100px]"></div>
        <div class="text-sm font-bold text-gray-500 border-b border-white/10 pb-2 mb-4 mt-8" data-i18n="wallet_history">🕒 ประวัติการเบิก (History)</div>
        <div id="wallet-history" class="space-y-2 pb-24 opacity-60"></div>
    </div>

    <div id="view-booking" class="hidden">
        <div class="flex justify-between mb-4"><h2 class="text-xl font-bold" data-i18n="bk_title">📝 จองโต๊ะ</h2><button onclick="show('view-customer')">✕</button></div>
        <div class="bg-white/5 p-6 rounded-2xl border border-white/10 space-y-3">
            ...
            <p id="date-display" class="text-xs text-[#fbbf24] text-right mt-[-8px] mb-2"></p>
            <input type="text" id="bk-pax" placeholder="จำนวนคน" class="glass-input" data-i18n-ph="ph_pax">
            <button onclick="submitBooking()" class="btn-action mt-2" data-i18n="btn_confirm_bk">ยืนยันการจอง</button>
        </div>
    </div>
    ...
    <div id="modal-deposit-success" class="fixed inset-0 z-[80] hidden flex items-center justify-center bg-black/95 p-6"><div class="bg-gray-900 w-full rounded-2xl p-6 text-center border border-green-500/30 overflow-y-auto max-h-screen"><div class="text-5xl mb-2">⏳</div><h2 class="text-2xl font-bold text-white mb-1">รอลูกค้ารับของ</h2><p class="text-gray-400 text-xs mb-4">ให้ลูกค้าสแกน QR หรือกรอกรหัสด้านล่าง</p><div class="bg-white/10 rounded-xl p-4 mb-4"><span id="success-code" class="text-6xl font-mono font-bold tracking-widest text-green-400">0000</span><p class="text-xs text-gray-500 mt-2">รหัสสำหรับลูกค้า</p></div><div class="bg-white p-3 rounded-xl w-fit mx-auto mb-4"><div id="success-qr"></div></div><button id="btn-copy-link" onclick="copyClaimLink()" class="w-full bg-blue-600/30 text-blue-400 border border-blue-500/50 py-2 rounded-lg mb-6 text-sm">📋 คัดลอกลิ้งค์ส่งให้ลูกค้า</button><button onclick="show('view-staff'); stopStaffCheck();" class="btn-action bg-gray-700 text-white">ปิดหน้าต่าง</button></div></div>
    ...
    <script>
        ...
        let currentLang = 'th';
        const I18N = { th: { welcome: "ยินดีต้อนรับ,", menu_booking: "จองโต๊ะ", menu_booking_sub: "Reservation", menu_wallet: "My Wallet", menu_wallet_sub: "ฝากเหล้า / เช็ครายการ", bk_title: "📝 จองโต๊ะ", ph_name: "ชื่อผู้จอง", ph_phone: "เบอร์โทรติดต่อ", ph_pax: "จำนวนคน (เช่น 5 คน)", btn_confirm_bk: "ยืนยันการจอง", wallet_title: "🍾 My Collection", wallet_active: "📦 รายการคงเหลือ (Active)", wallet_history: "🕒 ประวัติการเบิก (History)", back: "กลับ", btn_manual_code: "🔑 กรอกรหัสรับของ", alert_fill: "กรุณากรอกข้อมูลให้ครบ", alert_conf_title: "ยืนยันการจอง", alert_conf_head: "✨ ข้อมูลการจอง ✨", alert_name: "👤 ชื่อ:", alert_date: "📅 วันที่:", alert_pax: "👥 จำนวน:", alert_phone: "📞 โทร:", alert_time: "⏰ เวลา:", alert_note: "กรุณามารับโต๊ะก่อนเวลา", alert_note2: "เพื่อรักษาสิทธิ์ของท่าน" }, en: { welcome: "Welcome,", menu_booking: "Book Table", menu_booking_sub: "Reservation", menu_wallet: "My Wallet", menu_wallet_sub: "Bottle Keep / Items", bk_title: "📝 Reservation", ph_name: "Your Name", ph_phone: "Phone Number", ph_pax: "Pax (e.g. 5 ppl)", btn_confirm_bk: "Confirm Booking", wallet_title: "🍾 My Collection", wallet_active: "📦 Active Items", wallet_history: "🕒 History", back: "Back", btn_manual_code: "🔑 Enter Code", alert_fill: "Please fill all fields", alert_conf_title: "Confirm?", alert_conf_head: "✨ Reservation Info ✨", alert_name: "👤 Name:", alert_date: "📅 Date:", alert_pax: "👥 Pax:", alert_phone: "📞 Tel:", alert_time: "⏰ Time:", alert_note: "Please arrive before", alert_note2: "to hold your table." }, cn: { welcome: "欢迎,", menu_booking: "预订桌位", menu_booking_sub: "Reservation", menu_wallet: "我的钱包", menu_wallet_sub: "存酒 / 查看", bk_title: "📝 预订", ph_name: "姓名", ph_phone: "电话号码", ph_pax: "人数 (例如 5人)", btn_confirm_bk: "确认预订", wallet_title: "🍾 我的收藏", wallet_active: "📦 现有物品", wallet_history: "🕒 历史记录", back: "返回", btn_manual_code: "🔑 输入代码", alert_fill: "请填写完整信息", alert_conf_title: "确认?", alert_conf_head: "✨ 预订信息 ✨", alert_name: "👤 姓名:", alert_date: "📅 日期:", alert_pax: "👥 人数:", alert_phone: "📞 电话:", alert_time: "⏰ 时间:", alert_note: "请在", alert_note2: "前到达以保留座位。" } };
        ...
        async function main() { const dl = document.getElementById('drink-list'); DRINKS.forEach(d => { const op = document.createElement('option'); op.value = d; dl.appendChild(op); }); document.getElementById('bk-date').addEventListener('change', (e) => { document.getElementById('date-display').innerText = formatDateSmart(e.target.value); }); try { await liff.init({ liffId: CFG.liff }); if (!liff.isLoggedIn()) { if(isPreviewMode()) { profile={userId:"PREVIEW",displayName:"Preview User",pictureUrl:"https://via.placeholder.com/150"}; show('view-customer'); document.getElementById('loading').classList.add('hidden'); return; } liff.login({ redirectUri: window.location.href }); return; } profile = await liff.getProfile(); document.getElementById('user-img').src = profile.pictureUrl; document.getElementById('user-name').innerText = profile.displayName; const today = new Date(); document.getElementById('bk-date').valueAsDate = today; document.getElementById('date-display').innerText = formatDateSmart(today.toISOString().split('T')[0]); const u=new URLSearchParams(window.location.search); if(u.get('action')==='register'){show('view-staff-register');document.getElementById('loading').classList.add('hidden');return} if(u.get('claim_id'))await claimDeposit(u.get('claim_id')); checkRole(); } catch(e) { if(isPreviewMode()) { profile={userId:"PREVIEW",displayName:"Preview User",pictureUrl:"https://via.placeholder.com/150"}; show('view-customer'); document.getElementById('loading').classList.add('hidden'); } else Swal.fire('Error', e.message, 'error'); } }
        async function checkRole() { try { if(isPreviewMode()){document.getElementById('loading').classList.add('hidden');show('view-customer');return} const res=await fetch(CFG.worker+"/api/me?uid="+profile.userId);const data=await res.json();document.getElementById('loading').classList.add('hidden');if(data.status==='pending'){show('view-staff-waiting');document.getElementById('my-uid-display').innerText=profile.userId}else if(data.role==='staff'||data.role==='manager'){staffUser=data;document.getElementById('btn-staff-switch').classList.remove('hidden');sessionStorage.setItem('staffName',data.name);isStaffMode=true;updateModeUI()}else{show('view-customer')} } catch(e) { Swal.fire("Error", "Check URL", "error"); } }
        ...
        async function submitBooking() { const d = { n: document.getElementById('bk-name').value, p: document.getElementById('bk-phone').value, date: document.getElementById('bk-date').value, time: document.getElementById('bk-time').value, pax: document.getElementById('bk-pax').value }; if(!d.n || !d.p || !d.pax) return Swal.fire("แจ้งเตือน", I18N[currentLang].alert_fill, "warning"); const formattedDate = formatDateSmart(d.date); const txt = I18N[currentLang]; const msg = `${txt.alert_conf_head}\n━━━━━━━━━━━━━━━━━━\n${CFG.shop}\n${txt.alert_name} ${d.n}\n${txt.alert_date} ${formattedDate}\n${txt.alert_pax} ${d.pax}\n${txt.alert_phone} ${d.p}\n${txt.alert_time} ${d.time}\n\n${txt.alert_note} ${CFG.hold} น.\n${txt.alert_note2}\n━━━━━━━━━━━━━━━━━━`; if(liff.isInClient()) { await liff.sendMessages([{type:'text', text:msg}]); liff.closeWindow(); } else { Swal.fire("Simulated", msg, "info").then(()=> show('view-customer')); } }
        ...
        async function submitDeposit() { const file = document.getElementById('dep-file').files[0] || document.querySelector('input[type="file"]:not(#dep-file)').files[0]; const brand = document.getElementById('dep-brand').value; const amt = document.getElementById('dep-amount').value; const remark = document.getElementById('dep-remark').value; if(!file) return Swal.fire("แจ้งเตือน", "กรุณาถ่ายรูป", "warning"); if(!brand || !amt) return Swal.fire("แจ้งเตือน", "กรอกข้อมูลให้ครบ", "warning"); const btn = document.querySelector('#modal-deposit .btn-action'); const oldText = btn.innerText; btn.innerText = "กำลังย่อรูป..."; btn.disabled = true; if(isPreviewMode()){ setTimeout(()=>{show('modal-deposit-success');document.getElementById('success-code').innerText="0001";btn.innerText=oldText;btn.disabled=false},1000);return} const base64Img = await compressImage(file); btn.innerText = "กำลังบันทึก..."; try { const res = await fetch(CFG.worker + "/api/deposit", { method: 'POST', body: JSON.stringify({ staff_name: sessionStorage.getItem('staffName') || 'Staff', item_name: brand, item_type: depType, amount: amt, remarks: remark, image: base64Img }) }); const data = await res.json(); show('modal-deposit-success'); document.getElementById('success-code').innerText = data.code; document.getElementById('success-qr').innerHTML = ""; currentClaimLink = `https://liff.line.me/${CFG.liff}?claim_id=${data.deposit_id}`; new QRCode(document.getElementById("success-qr"), { text: currentClaimLink, width: 128, height: 128 }); startStaffCheck(data.deposit_id); } catch(e) { Swal.fire("Error", "Save Failed: " + e, "error"); } finally { btn.innerText = oldText; btn.disabled = false; } }
        ...
        function startStaffCheck(depositId) { if(staffCheckInterval) clearTimeout(staffCheckInterval); let attempt = 0; const check = async () => { attempt++; try { const res = await fetch(CFG.worker + "/api/check-deposit?id=" + depositId); const data = await res.json(); if (data.claimed) { Swal.fire({ title: '✅ เรียบร้อย!', text: 'ลูกค้าได้รับของแล้ว', icon: 'success' }).then(() => show('view-staff')); return; } } catch(e) {} let delay = 2000; if(attempt > 10) delay = 5000; if(attempt > 20) delay = 10000; if(attempt < 40) staffCheckInterval = setTimeout(check, delay); }; check(); }
        ...
        async function processWithdraw(code) { if(html5QrcodeScanner) { try { await html5QrcodeScanner.stop(); } catch(e){} html5QrcodeScanner = null; } const result = await Swal.fire({ title: 'ยืนยันการเบิก?', text: "รหัสรายการ: " + code, icon: 'question', showCancelButton: true, confirmButtonText: 'ยืนยัน', cancelButtonText: 'ยกเลิก' }); if(!result.isConfirmed) { closeModal('modal-withdraw'); return; } if(isPreviewMode()) { Swal.fire("สำเร็จ", "เบิกเรียบร้อย (Simulation)", "success").then(()=> closeModal('modal-withdraw')); return; } const res = await fetch(CFG.worker + "/api/withdraw", { method: 'POST', body: JSON.stringify({ code, staff: sessionStorage.getItem('staffName') }) }); const d = await res.json(); if(d.success) { Swal.fire("สำเร็จ", "เบิกเรียบร้อยแล้ว", "success").then(()=> closeModal('modal-withdraw')); } else { Swal.fire("ผิดพลาด", d.message, "error").then(()=> closeModal('modal-withdraw')); } }
        async function manualClaimInput() { const { value: code } = await Swal.fire({ title: I18N[currentLang].btn_manual_code, text: 'ดูเลข 4 หลักจากหน้าจอพนักงาน', input: 'text', inputPlaceholder: '0000', inputAttributes: { maxlength: 4, autocapitalize: 'off', autocorrect: 'off' }, showCancelButton: true }); if (code) { if(isPreviewMode()) { Swal.fire({title:'สำเร็จ!', text:'เข้ากระเป๋าเรียบร้อย (Simulation)', icon:'success'}).then(()=> window.location.reload()); return; } try { const res = await fetch(CFG.worker + "/api/claim", { method: 'POST', body: JSON.stringify({ code: code, uid: profile.userId, name: profile.displayName }) }); const d = await res.json(); if(d.success) Swal.fire({title:'สำเร็จ!', text:'เข้ากระเป๋าเรียบร้อย', icon:'success'}).then(()=> window.location.href = window.location.pathname); else Swal.fire("ผิดพลาด", d.message || "รหัสไม่ถูกต้อง", "error"); } catch(e) { Swal.fire("Error", "Connection Error", "error"); } } }
        ...
        async function loadMyWallet() { try { if(isPreviewMode()) return; const res = await fetch(CFG.worker + "/api/my-wallet?uid=" + profile.userId); const data = await res.json(); const activeDiv = document.getElementById('wallet-active'); const historyDiv = document.getElementById('wallet-history'); let activeHtml = "", historyHtml = ""; data.forEach(item => { const createdDate = new Date(item.created_at).toLocaleDateString('th-TH'); if (item.status === 'active') { const days = Math.ceil((new Date(item.expiry_date) - new Date()) / (1000 * 60 * 60 * 24)); const daysText = days > 0 ? `${days} วัน` : "หมดอายุ"; const colorClass = days <= 5 ? 'text-red-400' : 'text-green-400'; let imgUrl = (CFG.r2 && CFG.r2.length > 5) ? CFG.r2 + "/" + item.image_key : CFG.worker + "/api/image/" + item.image_key; let visual = item.item_type === 'liquor' ? `<div class="flex items-center gap-2 mt-2"><div class="flex-1 bg-gray-700 h-2 rounded-full overflow-hidden"><div class="${getProgressColor(item.amount)} h-full" style="width: ${Math.min(item.amount, 100)}%"></div></div><span class="text-xs font-bold text-[var(--accent)]">${item.amount}%</span></div>` : `<div class="mt-2 text-right"><span class="text-xl font-bold text-[var(--accent)]">x ${item.amount}</span> <span class="text-xs">ขวด</span></div>`; activeHtml += `<div class="bg-white/5 p-4 rounded-xl border border-white/10 relative overflow-hidden"><div class="flex gap-4"><div class="w-20 h-24 bg-black rounded-lg overflow-hidden shrink-0"><img src="${imgUrl}" class="w-full h-full object-cover" onclick="viewImage('${imgUrl}')"></div><div class="flex-1 min-w-0"><div class="flex justify-between items-start"><div><h3 class="font-bold text-lg truncate">${item.item_name}</h3><p class="text-xs text-gray-400">${item.remarks || '-'}</p></div><span class="bg-white/10 text-xs px-2 py-1 rounded font-mono">${item.deposit_code}</span></div>${visual}<div class="mt-2 flex justify-between items-end"><p class="text-xs ${colorClass}">⏳ เหลือ ${daysText}</p></div></div></div><div class="flex gap-2 mt-3 pt-3 border-t border-white/10"><button onclick="showWithdrawQR('${item.deposit_code}', '${item.id}')" class="flex-1 bg-red-500/10 text-red-400 py-2 rounded text-sm border border-red-500/30">เบิกออก</button><button onclick="shareItem('${item.id}','${item.item_name}', '${imgUrl}', '${daysText}')" class="flex-1 bg-blue-500/10 text-blue-400 py-2 rounded text-sm border border-blue-500/30">โอนสิทธิ์</button></div></div>`; } else if (item.status === 'withdrawn') { historyHtml += `<div class="flex justify-between items-center bg-white/5 p-3 rounded-lg border border-white/10 grayscale"><div class="text-xs text-gray-400"><div>${item.item_name}</div><div class="text-[10px] opacity-70">ฝากเมื่อ: ${createdDate}</div></div><div class="text-xs font-bold text-red-500 border border-red-500 px-2 py-1 rounded">เบิกแล้ว</div></div>`; } }); if (activeDiv.innerHTML !== activeHtml) activeDiv.innerHTML = activeHtml; if (historyDiv.innerHTML !== historyHtml) historyDiv.innerHTML = historyHtml; } catch(e) { console.log(e); } }
        function showWithdrawQR(code, itemId) { let timeLeft = 30; Swal.fire({ title: 'Scan Code: ' + code, html: `<div class="flex justify-center my-4 p-4 bg-white rounded-xl"><div id="wd-qr"></div></div><p class="text-gray-400 text-sm">QR หมดอายุใน <span id="qr-countdown" class="text-red-500 font-bold text-xl">30</span> วิ</p><p class="text-xs text-gray-500 mt-2">ยื่นให้พนักงานสแกน</p>`, showConfirmButton: false, showCloseButton: true, didOpen: () => { new QRCode(document.getElementById("wd-qr"), { text: code, width: 200, height: 200 }); qrTimer = setInterval(() => { timeLeft--; const el = document.getElementById('qr-countdown'); if(el) el.innerText = timeLeft; if(timeLeft <= 0) { clearInterval(qrTimer); clearInterval(qrPoll); Swal.close(); Swal.fire("หมดเวลา", "QR Code หมดอายุ กรุณากดใหม่", "warning"); } }, 1000); let pollAttempt = 0; const pollWithdraw = async () => { pollAttempt++; const res = await fetch(CFG.worker + "/api/my-wallet?uid=" + profile.userId); const data = await res.json(); if (!data.find(i => i.id == itemId && i.status === 'active')) { clearInterval(qrTimer); clearInterval(qrPoll); Swal.fire("สำเร็จ", "รายการถูกเบิกเรียบร้อยแล้ว", "success").then(() => loadMyWallet()); } let delay = 2000; if(pollAttempt>10) delay=5000; if(pollAttempt>30) delay=10000; if(timeLeft > 0) qrPoll = setTimeout(pollWithdraw, delay); }; pollWithdraw(); }, willClose: () => { clearInterval(qrTimer); clearInterval(qrPoll); } }); }
        ...
        async function loadManagerStock() { const res = await fetch(CFG.worker + "/api/manager-stock"); allStockData = await res.json(); filterStock(); }
        ...
        async function loadManagerData() { const res = await fetch(CFG.worker + "/api/manager-list"); const data = await res.json(); const pList = document.getElementById('pending-list'); pList.innerHTML = ""; data.pending.forEach(s => { pList.innerHTML += `<div class="flex justify-between py-2 border-b border-white/10"><span>${s.name}</span><button onclick="staffAction('approve','${s.user_id}','${s.name}')" class="text-green-400 font-bold">✓</button></div>`; }); const aList = document.getElementById('active-list'); aList.innerHTML = ""; data.active.forEach(s => { aList.innerHTML += `<div class="flex justify-between py-2 border-b border-white/10"><span>${s.name}</span><button onclick="staffAction('remove','${s.user_id}','${s.name}')" class="text-red-400 font-bold">✕</button></div>`; }); }
        async function loadHistoryLogs() { const res = await fetch(CFG.worker + "/api/logs"); const data = await res.json(); const list = document.getElementById('logs-list'); list.innerHTML = ""; data.logs.forEach(l => { const color = l.action === 'deposit' ? 'text-blue-400' : (l.action === 'withdraw' ? 'text-red-400' : 'text-gray-400'); const date = new Date(l.created_at).toLocaleString('th-TH'); let imgHTML = l.image_key ? `<div class="mt-1"><img src="${(CFG.r2 && CFG.r2.length > 5) ? CFG.r2 + "/" + l.image_key : CFG.worker + "/api/image/" + l.image_key}" class="h-12 rounded border border-white/20" onclick="viewImage(this.src)" onerror="this.parentElement.style.display='none'"></div>` : ""; list.innerHTML += `<div class="py-3 border-b border-white/5"><div class="flex justify-between"><strong class="${color} uppercase">${l.action}</strong><span class="opacity-50">${date}</span></div><div class="text-gray-300 mt-1">${l.details}</div>${imgHTML}<div class="text-xs opacity-50 mt-1">by ${l.staff_name}</div></div>`; }); }
...
export default {
  async fetch(request, env, ctx) {
    ...
    async function addLog(action, staff, details, imgKey = null) { try { await env.DB.prepare("INSERT INTO logs (action, staff_name, details, image_key) VALUES (?, ?, ?, ?)").bind(action, staff, details, imgKey).run(); } catch(e){} }
    try {
        if (path.startsWith("/api/image/")) {
            const key = path.split('/').pop(); const object = await env.BUCKET.get(key);
            if (!object) return new Response('Not Found', { status: 404 });
            const headers = new Headers(); object.writeHttpMetadata(headers); headers.set('etag', object.httpEtag);
            return new Response(object.body, { headers });
        }
        if (path === "/api/me") {
            const uid = url.searchParams.get('uid'); const user = await env.DB.prepare("SELECT * FROM staff_access WHERE user_id = ?").bind(uid).first();
        ...
        if (path === "/api/login" && request.method === "POST") {
            ...
            return new Response(JSON.stringify({ success: body.pass === "__MGR_PASS__" }), { headers: corsHeaders });
        }
        if (path === "/api/register" && request.method === "POST") {
            const body = await request.json(); await env.DB.prepare("INSERT OR IGNORE INTO staff_access (user_id, name, status) VALUES (?, ?, 'pending')").bind(body.uid, body.name).run();
            return new Response(JSON.stringify({ success: true }), { headers: corsHeaders });
        }
        if (path === "/api/deposit" && request.method === "POST") {
            const body = await request.json();
            const imgString = body.image.split(',')[1]; const imgBuffer = Uint8Array.from(atob(imgString), c => c.charCodeAt(0)); const filename = `dep_${Date.now()}.jpg`;
            ...
            let nextNum = 1; if (lastItem && lastItem.deposit_code) { const parsed = parseInt(lastItem.deposit_code, 10); if (!isNaN(parsed)) nextNum = parsed + 1; }
        ...
        if (path === "/api/claim" && request.method === "POST") {
            ...
            if (body.code) res = await env.DB.prepare("UPDATE deposits SET owner_uid = ?, owner_name = ?, status = 'active' WHERE deposit_code = ? AND status = 'pending_claim'").bind(body.uid, ownerName, body.code).run();
        ...
        if (path === "/api/staff-action" && request.method === "POST") {
            const body = await request.json();
            if (body.action === 'approve') { await env.DB.prepare("UPDATE staff_access SET status = 'active' WHERE user_id = ?").bind(body.uid).run(); await addLog('approve', 'Manager', `Approved: ${body.name}`); }
            else if (body.action === 'remove') { await env.DB.prepare("DELETE FROM staff_access WHERE user_id = ?").bind(body.uid).run(); await addLog('remove', 'Manager', `Removed: ${body.name}`); }
            return new Response(JSON.stringify({ success: true }), { headers: corsHeaders });
        }
    } catch(e) { return new Response(JSON.stringify({ error: e.message }), { status: 500, headers: corsHeaders }); }
    return new Response("API Ready v43", { headers: corsHeaders });
  },
  async scheduled(event, env, ctx) {
    ...
    const results = await env.DB.prepare(query).all(); const items = results.results; if (items.length === 0) return;
    const userItems = {}; items.forEach(item => { if (!item.owner_uid) return; if (!userItems[item.owner_uid]) userItems[item.owner_uid] = []; userItems[item.owner_uid].push(item); });
...
async function sendLinePush(uid, items, token) {
    ...
    const bubbles = items.map(item => {
        ...
        return { type: "bubble", body: { type: "box", layout: "vertical", contents: [{ type: "text", text: "⚠️ แจ้งเตือนวันหมดอายุ", weight: "bold", color: color, size: "sm" }, { type: "text", text: item.item_name, weight: "bold", size: "xl", margin: "md", wrap: true }, { type: "text", text: `จะหมดอายุในอีก ${daysLeft} วัน`, size: "sm", color: "#aaaaaa", margin: "sm" }] }, footer: { type: "box", layout: "vertical", contents: [{ type: "button", style: "primary", action: { type: "uri", label: "เปิดกระเป๋า", uri: "https://liff.line.me/__LIFF__" }, color: color }] } };
    });
    await fetch("https://api.line.me/v2/bot/message/push", { method: "POST", headers: { "Content-Type": "application/json", "Authorization": "Bearer " + token }, body: JSON.stringify({ to: uid, messages: [{ type: "flex", altText: "แจ้งเตือนวันหมดอายุ", contents: { type": "carousel", contents: bubbles } }] }) });
}
"""
...
class AppGenerator(ctk.CTk):
    def __init__(self):
        super().__init__()
        self.title("Nightlife System Builder V43 (Original UI Enforced)")
        ...
        # 8 PRESET LAYOUTS (Standard = V23 Hardcoded in HTML)
        self.LAYOUTS = {
            "Standard (V23 Original)": "", 
            "Grid (2 Columns)": ".menu-layout{display:grid;grid-template-columns:1fr 1fr;gap:15px}.btn-menu{display:flex;flex-direction:column;align-items:center;justify-content:center;padding:20px;aspect-ratio:1/1;text-align:center;margin-bottom:0;background:linear-gradient(135deg,var(--card-grad-start),var(--card-grad-end))!important;border:1px solid rgba(255,255,255,0.1)!important;box-shadow:none!important}.icon{margin-bottom:8px;font-size:35px!important}.label{font-size:18px!important}",
            "Wide List (Row)": ".menu-layout{display:flex;flex-direction:column;gap:12px}.btn-menu{display:flex!important;flex-direction:row!important;align-items:center;justify-content:flex-start!important;padding:20px 25px!important;text-align:left!important;margin-bottom:0;background:linear-gradient(135deg,var(--card-grad-start),var(--card-grad-end))!important}.icon{margin-bottom:0!important;margin-right:25px;font-size:32px!important}.text-wrap{text-align:left}.label{font-size:18px!important}.sub-label{font-size:14px!important}",
            "Large Tiles (1 Col)": ".menu-layout{display:flex;flex-direction:column;gap:20px}.btn-menu{padding:50px 20px!important;display:flex;flex-direction:column;align-items:center;text-align:center;margin-bottom:0;background:linear-gradient(135deg,var(--card-grad-start),var(--card-grad-end))!important}.icon{font-size:60px!important;margin-bottom:15px!important}.label{font-size:28px!important}",
            "Circle Icons (3 Col)": ".menu-layout{display:grid;grid-template-columns:1fr 1fr 1fr;gap:10px}.btn-menu{aspect-ratio:1/1!important;border-radius:50%!important;display:flex;flex-direction:column;justify-content:center;align-items:center;padding:10px;text-align:center;margin-bottom:0;background:linear-gradient(135deg,var(--card-grad-start),var(--card-grad-end))!important}.icon{font-size:30px!important;margin-bottom:5px!important}.label{font-size:10px!important}.sub-label{display:none}",
            "Dense Grid (3 Col)": ".menu-layout{display:grid;grid-template-columns:1fr 1fr 1fr;gap:8px}.btn-menu{padding:15px 5px!important;display:flex;flex-direction:column;align-items:center;text-align:center;margin-bottom:0;background:linear-gradient(135deg,var(--card-grad-start),var(--card-grad-end))!important}.icon{font-size:25px!important}.label{font-size:12px!important}.sub-label{font-size:9px!important}",
            "Hero Header": ".menu-layout{display:grid;grid-template-columns:1fr 1fr;gap:15px}.menu-item-1{grid-column:1/-1;aspect-ratio:2/1;background:linear-gradient(45deg,var(--accent),#fff)!important;color:#000!important;display:flex;flex-direction:column;align-items:center;justify-content:center}.menu-item-1 .label{color:#000!important;font-size:24px!important}.menu-item-1 .sub-label{color:#333!important}.menu-item-1 .icon{font-size:50px!important}.btn-menu{display:flex;flex-direction:column;align-items:center;text-align:center;background:linear-gradient(135deg,var(--card-grad-start),var(--card-grad-end))!important}",
            "Alternating List": ".menu-layout{display:flex;flex-direction:column;gap:15px}.menu-item-2{flex-direction:row-reverse!important;text-align:right!important}.btn-menu{display:flex!important;flex-direction:row!important;align-items:center;justify-content:space-between!important;padding:25px;margin-bottom:0;background:linear-gradient(135deg,var(--card-grad-start),var(--card-grad-end))!important}.icon{font-size:30px!important;margin:0!important}",
            "Minimal Text": ".menu-layout{display:flex;flex-direction:column;gap:0px;background:rgba(255,255,255,0.05);border-radius:16px;overflow:hidden;border:1px solid rgba(255,255,255,0.1)}.btn-menu{border-radius:0!important;background:transparent!important;border:none!important;border-bottom:1px solid rgba(255,255,255,0.1)!important;box-shadow:none!important;flex-direction:row!important;justify-content:flex-start!important;padding:20px!important;display:flex;align-items:center;margin-bottom:0}.btn-menu:last-child{border-bottom:none!important}.icon{font-size:24px!important;margin-right:20px;margin-bottom:0!important}.text-wrap{text-align:left}"
        }
    ...
    def setup_ui(self):
        ...
        self.combo_layout.set("Standard (V23 Original)")
        ...
        self.entries = {}
        for k, h in [("Shop Name", "ชื่อร้าน"), ("LIFF ID", "165xxxx-xxxx"), ("Worker URL", "Cloudflare Worker"), ("Manager Password", "รหัสผ่าน"), ("R2 Public URL", "Optional"), ("Channel Access Token", "Line Messaging API Token"), ("Hold Table Time", "เวลาหลุดจอง (เช่น 20:30)")]:
        ...
        ctk.CTkLabel(self.preview, text="Preview V43", text_color="#555").pack(pady=20)
    ...
    def update_preview_ui(self, event=None):
        ...
        elif "Grid" in layout: self.p_card.configure(height=120, width=140)
    ...
    def load_config(self):
        if os.path.exists("config.json"):
            try: 
                ...
                if 'layout' in d: 
                    val = d['layout']
                    if val == "Standard Grid": val = "Standard (V23 Original)"
                    self.combo_layout.set(val)
