...
# ==============================================================================
# 1. HTML TEMPLATE (V56 - FULL EXPANDED VERSION)
# ==============================================================================
...
<head>
    ...
    <style>
        ...
        /* --- V23 BUTTON STYLE --- */
        .btn-menu {
            background: linear-gradient(135deg, #0e4296, #1e293b) !important;
        ...
        .btn-menu:hover {
            box-shadow: 0 10px 25px rgba(14,66,150,0.4) !important;
        ...
        /* --- UTILS --- */
        .glass-input {
            background: rgba(0,0,0,0.3); border: 1px solid rgba(255,255,255,0.1); border-radius: 16px; padding: 12px; width: 100%; color: white; outline: none; margin-bottom: 10px; font-size: 16px;
        }
        .glass-input:focus { border-color: #0e4296; background: rgba(0,0,0,0.5); }
        .glass-input::placeholder { color: #94a3b8; }

        .btn-action {
            background: #0e4296; color: white; font-weight: bold; padding: 14px; border-radius: 16px; width: 100%; border: none; font-size: 16px; box-shadow: 0 0 10px rgba(14,66,150,0.5); cursor: pointer;
        }
        
        .hidden { display: none !important; }
        
        .lang-select {
            background: rgba(255,255,255,0.05); color: #94a3b8; border: 1px solid rgba(255,255,255,0.1); border-radius: 20px; padding: 5px 10px; font-size: 12px; outline: none; margin-left: auto;
        }
        .lang-select option { background: #0f172a; color: white; }

        .swal2-popup { background: #1e293b !important; border: 1px solid rgba(255,255,255,0.1); border-radius: 16px !important; color: white !important; width: 90% !important; }
        .swal2-title { color: white !important; }
        .swal2-confirm { background-color: #0e4296 !important; border-radius: 8px !important; }
        .swal2-cancel { background-color: #334155 !important; border-radius: 8px !important; }

        .file-upload { border: 2px dashed rgba(255,255,255,0.3); border-radius: 16px; padding: 20px; text-align: center; margin-bottom: 10px; cursor: pointer; }
        .file-upload.has-img { border-style: solid; border-color: #0e4296; padding: 0; overflow: hidden; }
        .preview-img { width: 100%; height: 200px; object-fit: cover; }
        .type-btn { flex: 1; padding: 12px; border-radius: 16px; border: 1px solid rgba(255,255,255,0.1); background: rgba(255,255,255,0.05); color: #aaa; transition: all 0.2s; }
        .type-btn.active { background: #0e4296; color: white; font-weight: bold; border-color: #0e4296; box-shadow: 0 0 10px rgba(14,66,150,0.4); }
...
<body>
    <div class="main-container">
        <div id="loading" class="fixed inset-0 z-50 flex flex-col items-center justify-center bg-black/90 backdrop-blur-sm">
            <div class="animate-spin rounded-full h-12 w-12 border-4 border-t-transparent" style="border-color: #0e4296;"></div>
            <p class="mt-4 text-sm" style="color: #94a3b8;">Loading...</p>
        </div>

        <div class="text-center mb-8 flex justify-between items-start">
            <div onclick="secretTap()" class="flex-1 text-left">
                <h1 class="text-2xl font-bold text-white mb-1">__SHOP__</h1>
                <p class="text-gray-400 text-sm">Nightlife System</p>
            </div>
            <div class="flex flex-col gap-2 items-end">
                <div class="flex gap-2">
                    ...
                    <button onclick="verifyManager()" class="opacity-50 hover:opacity-100 text-lg">⚙️</button>
                </div>
            </div>
        </div>
        ...
        <div id="view-customer" class="hidden animate-fade-in">
             <div class="text-center mb-6">
                <div class="inline-block p-1 rounded-full border-2 border-[#0e4296] mb-2">
                    <img id="user-img" class="w-16 h-16 rounded-full" src="">
                </div>
                <h2 id="user-name" class="text-xl font-bold">Guest</h2>
                <p class="text-gray-400 text-xs" data-i18n="welcome">ยินดีต้อนรับ กรุณาเลือกบริการ</p>
             </div>
             ...
             <div id="menu-container" class="menu-layout">
                <div onclick="show('view-booking')" class="btn-menu menu-item-1">
                    ...
                    <span class="sub-label" data-i18n="menu_booking_sub">สำรองที่นั่งล่วงหน้า</span>
                </div>
        ...
        <div id="view-staff" class="hidden">
            <div class="staff-zone" style="display:block !important;">
                <div style="text-align: center; margin-bottom: 20px;">
                    <span style="color:#ef4444; font-weight:bold; font-size:14px; letter-spacing:2px; text-transform:uppercase; background:#1e293b; padding:5px 10px; border-radius: 8px;">STAFF ONLY</span>
                </div>
                <div class="grid grid-cols-2 gap-4">
                    <div onclick="openDepositFlow()" class="btn-menu" style="background: #1e293b !important; padding: 20px !important; margin-bottom: 0 !important;">
                        <span class="icon" style="font-size: 30px !important;">📥</span><span class="label" style="font-size: 16px !important;">รับฝาก</span>
                    </div>
                    <div onclick="openWithdrawScanner()" class="btn-menu" style="background: linear-gradient(135deg, #7f1d1d, #450a0a) !important; border-color: #ef4444 !important; padding: 20px !important; margin-bottom: 0 !important;">
                        <span class="icon" style="font-size: 30px !important;">📤</span><span class="label text-red-400" style="font-size: 16px !important;">เบิกออก</span>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div id="view-wallet" class="hidden">
            <div class="flex justify-between mb-4">
                <button onclick="stopWalletSync(); show('view-customer')" class="text-sm flex items-center gap-1 text-gray-400">← <span data-i18n="back">Back</span></button>
                <button onclick="manualClaimInput()" class="text-xs bg-[#0e4296] text-white px-3 py-1 rounded-full font-bold" data-i18n="btn_manual_code">🔑 กรอกรหัส</button>
            </div>
            <h2 class="text-xl font-bold mb-4" data-i18n="wallet_title">🍾 My Collection</h2>
            <div class="text-sm font-bold text-[#0e4296] border-b border-white/10 pb-2 mb-4" data-i18n="wallet_active">📦 รายการคงเหลือ (Active)</div>
        ...
        <div id="view-booking" class="hidden">
            <div class="flex justify-between mb-4"><h2 class="text-xl font-bold" data-i18n="bk_title">📝 จองโต๊ะ</h2><button onclick="show('view-customer')" class="text-gray-400">✕</button></div>
            <div class="bg-white/5 p-6 rounded-2xl border border-white/10 space-y-3">
                ...
                <p id="date-display" class="text-xs text-[#0e4296] text-right mt-[-8px] mb-2"></p>
        ...
        <div id="modal-deposit-success" class="fixed inset-0 z-[80] hidden flex items-center justify-center bg-black/95 p-6">
            <div class="bg-gray-900 w-full rounded-2xl p-6 text-center border border-green-500/30 overflow-y-auto max-h-screen">
                <div class="text-5xl mb-2">⏳</div><h2 class="text-2xl font-bold text-white mb-1">รอลูกค้ารับของ</h2><p class="text-gray-400 text-xs mb-4">ให้ลูกค้าสแกน QR หรือกรอกรหัส</p>
        ...
        <div id="modal-withdraw" class="fixed inset-0 z-[70] hidden flex-col bg-black p-4">
            ...
            <div class="flex gap-2"><input id="manual-code" placeholder="รหัส 4 หลัก" class="glass-input text-center text-xl tracking-widest font-mono"><button onclick="manualWithdraw()" class="btn-action w-auto px-6">เบิก</button></div>
        </div>

        <div id="view-manager" class="hidden">
            <div class="flex justify-between items-center mb-4"><h2 class="text-xl font-bold text-white">👑 Dashboard</h2><button onclick="location.reload()" class="text-xs bg-red-900 text-red-200 px-3 py-1 rounded">Logout</button></div>
            ...
            <div id="mgr-tab-stock" class="space-y-4">
                <div class="bg-white/5 p-3 rounded-xl border border-white/10 flex flex-wrap gap-2">
                    <div class="flex w-full gap-2"><input id="filter-brand" placeholder="🔍 ค้นหา..." class="bg-black/30 text-white text-sm px-3 py-2 rounded-lg flex-1 outline-none border border-white/10"><button onclick="filterStock()" class="bg-[#0e4296] text-white px-4 py-2 rounded-lg font-bold">ค้นหา</button></div>
                    <select id="filter-expiry" onchange="filterStock()" class="bg-black/30 text-white text-sm px-3 py-2 rounded-lg outline-none border border-white/10"><option value="all">📅 วันหมดอายุ</option><option value="7">⚠️ < 7 วัน</option><option value="14">⚠️ < 14 วัน</option></select>
                    <select id="filter-pct" onchange="filterStock()" class="bg-black/30 text-white text-sm px-3 py-2 rounded-lg outline-none border border-white/10"><option value="all">📊 ปริมาณ</option><option value="low">🔴 น้อย</option><option value="mid">🟡 กลาง</option><option value="high">🟢 เยอะ</option></select>
                </div>
                <button onclick="testNotification()" class="w-full bg-orange-600/20 border border-orange-500/50 text-orange-400 py-3 rounded-xl font-bold hover:bg-orange-600/40 transition">🔔 Test Notification (ระบุ UID)</button>
                <div id="stock-list" class="space-y-2 h-[350px] overflow-y-auto pb-10">Loading Stock...</div>
            </div>
    ...
    <script>
                    ...
                    const pollWithdraw = async () => {
                        ...
                        try {
                            ...
                            if (!data.find(i => i.id == itemId && i.status === 'active')) {
                                ...
                                Swal.fire({title: "สำเร็จ", text: "รายการถูกเบิกเรียบร้อยแล้ว", icon: "success", timer: 2000, showConfirmButton: false}).then(() => loadMyWallet());
                                return;
                            }
                        } catch(e) {}
                        let delay = 2000;
                        if(pollAttempt > 10) delay = 5000;
                        if(timeLeft > 0) qrPoll = setTimeout(pollWithdraw, delay);
                    };
                    pollWithdraw();
                },
                willClose: () => { clearInterval(qrTimer); clearTimeout(qrPoll); }
            });
        }
        ...
        async function testNotification() {
            if(isPreviewMode()) { Swal.fire("Sent!", "Test notification simulated.", "success"); return; }
            let targetUid = profile.userId;
            
            // ALWAYS ASK FOR UID IN MANAGER MODE (To allow testing with real UID on PC)
            const { value: manualUid } = await Swal.fire({
                title: 'Test Notification',
                text: 'กรุณากรอก User ID ของคุณ (ที่ขึ้นต้นด้วย U...)',
                input: 'text',
                inputValue: targetUid && targetUid !== 'PREVIEW' ? targetUid : '',
                inputPlaceholder: 'Uxxxxxxxxxxxxxxxxxxxx...',
                showCancelButton: true,
                confirmButtonText: '🚀 ส่งข้อความทดสอบ'
            });
            
            if (manualUid) targetUid = manualUid;
            else return; // Cancelled

            try {
                const res = await fetch(CFG.worker + "/api/test-push", { method: 'POST', body: JSON.stringify({ uid: targetUid }) });
                const d = await res.json();
                if(d.success) Swal.fire("สำเร็จ!", "ส่งข้อความแล้ว เช็คใน LINE ได้เลย", "success");
        ...
        async function loadManagerStock() { 
            try {
                ...
                document.getElementById('stock-list').innerHTML = `<p class="text-red-500 text-center">โหลดข้อมูลไม่สำเร็จ (Error: ${e.message})<br>โปรดตรวจสอบ Database</p>`;
            }
        }
        ...
        async function loadHistoryLogs() { 
            try {
                ...
                document.getElementById('logs-list').innerHTML = "<span class='text-red-500'>Error loading logs.</span>";
            }
        }

        function renderLogs() {
            const list = document.getElementById('logs-list'); const showBtn = document.getElementById('btn-load-more-logs'); list.innerHTML = ""; 
            const logsToShow = allLogs.slice(0, logsLimit);
            ...
            if (logsLimit < allLogs.length) { showBtn.classList.remove('hidden'); showBtn.innerText = `⬇️ โหลดเพิ่มเติม (${allLogs.length - logsLimit})`; } else { showBtn.classList.add('hidden'); }
        }
...
# ==============================================================================
# 2. WORKER CODE (V55 - VERBOSE DEBUGGING)
# ==============================================================================
WORKER_RAW = r"""
export default {
    async fetch(request, env, ctx) {
      ...
      try {
          ...
          // --- DEBUG TEST NOTIFICATION (V55 - Verbose Error) ---
          if (path === "/api/test-push" && request.method === "POST") {
              const body = await request.json();
              if (!env.LINE_TOKEN) return new Response(JSON.stringify({ success: false, message: "No LINE Token in Secrets" }), { headers: corsHeaders });
              
              const mockName = "Black Label (Simulated)";
              const mockDays = 3;
              const mockColor = "#f97316";
              const bubble = {
                  "type": "bubble",
                  "body": {
                      "type": "box", "layout": "vertical",
                      "contents": [
                          { "type": "text", "text": "🔔 แจ้งเตือนทดสอบ (Debug)", "weight": "bold", "color": "#1DB446", "size": "sm" },
                          { "type": "text", "text": "ถ้าเห็นข้อความนี้ แสดงว่าระบบพร้อมทำงานแล้วครับ!", "wrap": true, "margin": "md" }
                      ]
                  },
                  "footer": {
                      "type": "box", "layout": "vertical",
                      "contents": [
                          { "type": "button", "style": "primary", "action": { "type": "uri", "label": "เปิดกระเป๋า", "uri": "https://liff.line.me/__LIFF__" }, "color": mockColor }
                      ]
                  }
              };
              ...
              const lineResp = await fetch("https://api.line.me/v2/bot/message/push", {
                  ...
                  body: JSON.stringify({
                      "to": body.uid,
                      "messages": [{ "type": "flex", "altText": "Test Connection", "contents": { "type": "carousel", "contents": [bubble] } }]
                  })
              });

              const lineData = await lineResp.json();
              if (!lineResp.ok) {
                  return new Response(JSON.stringify({ success: false, message: "LINE Error: " + lineData.message, details: lineData.details }), { headers: corsHeaders });
              }
              return new Response(JSON.stringify({ success: true, line_response: lineData }), { headers: corsHeaders });
          }
          
      } catch(e) { return new Response(JSON.stringify({ error: e.message }), { status: 500, headers: corsHeaders }); }
      return new Response("API Ready v55", { headers: corsHeaders });
    },
...
class AppGenerator(ctk.CTk):
    def __init__(self):
        super().__init__()
        self.title("Nightlife System Builder V57 (Bug Fixed)")
    ...
    def setup_ui(self):
        ...
        ctk.CTkLabel(self.preview, text="Preview V57 (Fixed)", text_color="#555").pack(pady=20)
    ...
    def update_preview_ui(self, event=None):
        ...
        # --- FIXED TYPO HERE (layout_key -> layout) ---
        layout = self.combo_layout.get()
        layout_css = self.LAYOUTS.get(layout, "")
    ...
    def open_live_preview(self):
        try:
            ...
            radius = self.vars.get('radius', '16px'); shadow = self.vars.get('shadow', '0 4px 15px rgba(0,0,0,0.3)'); border = self.vars.get('border', '1px solid rgba(255,255,255,0.1)')
