...
# ==============================================================================
# 1. HTML TEMPLATE (V19 - SECRET TAP & FIXED LOGIN REDIRECT)
# ==============================================================================
...
<body class="p-6 max-w-md mx-auto w-full relative">
    ...
    <div id="view-booking" class="hidden">
        ...
        <div class="bg-white/5 p-6 rounded-2xl border border-white/10 space-y-3">
            ...
            <input type="text" id="bk-pax" placeholder="จำนวนคน (เช่น 5 คน)" class="glass-input">
    ...
    <div id="modal-deposit" class="fixed inset-0 z-[70] hidden flex-col bg-black overflow-y-auto p-4">
        ...
        <label class="file-upload" id="dep-img-area">
            ...
            <div id="dep-placeholder">
                ...
                <p class="text-sm text-gray-400">แตะเพื่อเปิดกล้อง</p>
            </div>
            <img id="dep-preview" class="hidden preview-img">
        </label>
        
        <div class="flex justify-end mb-4">
             <label class="text-xs text-gray-400 underline cursor-pointer">
                 เลือกรูปจากอัลบั้ม
                 <input type="file" accept="image/*" class="hidden" onchange="previewDepositImage(this)">
             </label>
        </div>
    ...
    <div id="modal-withdraw" class="fixed inset-0 z-[70] hidden flex-col bg-black p-4">
        ...
        <label class="block text-center mb-4">
             <span class="text-xs text-gray-400 bg-white/10 px-3 py-1 rounded-full cursor-pointer">📂 อัปโหลดรูป QR (ถ้าสแกนไม่ได้)</span>
    ...
    <div id="view-staff-register" class="fixed inset-0 z-[65] hidden flex items-center justify-center bg-black/90 p-6">
        <div class="bg-[#1e293b] w-full max-w-xs p-6 rounded-2xl border border-white/10 text-center">
            ...
            <p class="text-xs text-gray-400 mb-4">สมัครพนักงานใหม่</p>
    ...
    <script>
        ...
        let allStockData = []; 
        let secretTapCount = 0; // For secret tap

        // --- DRINK DATABASE ---
        const DRINKS = [
            "Chang", "Singha", "Leo", "Heineken", "Tiger", "Budweiser", 
            "Hoegaarden", "Hoegaarden Rosée", "Stella Artois", "San Miguel", "San Miguel Light", 
            "Federbräu", "Snowy Weizen", "Copper", "Corona", "Asahi", "Carlsberg",
            "Regency", "SangSom", "Blend 285", "Blend 285 Signature", "Hong Thong", "Meridian", 
            "Johnnie Walker Red", "Johnnie Walker Black", "Johnnie Walker Double Black", "Johnnie Walker Gold", "Johnnie Walker Blue", 
            "Chivas Regal 12", "Chivas Regal 18", "Jack Daniel’s", "Jameson", "Ballantine's", 
            "Monkey Shoulder", "Absolut Vodka", "Grey Goose", "Suntory Kakubin", "Bombay Sapphire", "Jose Cuervo",
            "Penfolds Bin 2", "Penfolds Bin 389", "Penfolds Bin 407", "Yellow Tail", "Jacob's Creek", 
            "Casillero del Diablo", "Mont Gras", "Robert Mondavi", "Concha y Toro", "Silver Oak", "Sutter Home", "Berri Estates"
        ];
        ...
        async function main() {
            try {
                ...
                if (!liff.isLoggedIn()) { 
                    // FIXED: Persist Query Params on Login Redirect
                ...
                if (urlParams.get('action') === 'register') {
                    show('view-staff-register');
                    document.getElementById('loading').classList.add('hidden');
                    return; // Stop normal flow to show register
                }
        ...
        function updateModeUI() {
            ...
            if (btnText) {
                btnText.innerText = isStaffMode ? "Switch to Personal" : "Back to Work"; 
        ...
        function previewDepositImage(input) {
            if (input.files && input.files[0]) {
                ...
                reader.onload = function(e) {
                    ...
                    document.getElementById('dep-img-area').classList.add('has-img');
                }
        ...
        function setDepType(t) { 
            ...
            document.getElementById('dep-amount').placeholder = (t === 'liquor') ? "จำนวนเหลือ (%)" : "จำนวน (ขวด)";
        }
        ...
        async function submitDeposit() {
            ...
            // -- NEW: Image Compression & UI Feedback --
            const btn = document.querySelector('#modal-deposit .btn-action'); 
            const oldText = btn.innerText; 
            btn.innerText = "กำลังย่อรูป..."; 
            btn.disabled = true;

            const base64Img = await compressImage(fileToUse);
            ...
            try {
                ...
                // Start Polling to check if customer claimed
        ...
        function startStaffCheck(depositId) {
            ...
            const check = async () => {
                ...
                if(attempt < 40) staffCheckInterval = setTimeout(check, delay); 
            };
            check();
        }
        ...
        function startScanner() {
            ...
            html5QrcodeScanner.start({ facingMode: "environment" }, config, (decodedText) => {
                 processWithdraw(decodedText);
            }, (errorMessage) => { }).catch(err => {
                document.getElementById('reader').innerHTML = "<p class='text-red-500 text-center pt-10'>ไม่สามารถเปิดกล้องได้<br>โปรดใช้การอัปโหลดรูป หรือ กรอกรหัส</p>";
            });
        }

        async function cleanFromFile(input) {
            ...
            try {
                ...
                processWithdraw(decodedText);
            } catch (err) { Swal.fire("Error", "สแกนไม่พบ QR Code", "error"); }
        }
        ...
        async function processWithdraw(code) {
            ...
            const res = await fetch(CFG.worker + "/api/withdraw", { 
                method: 'POST', body: JSON.stringify({ code, staff: sessionStorage.getItem('staffName') }) 
            });
            const d = await res.json();
            
            if(d.success) { 
                Swal.fire("สำเร็จ", "เบิกเรียบร้อยแล้ว", "success").then(()=> closeModal('modal-withdraw')); 
            } else { 
                Swal.fire("ผิดพลาด", d.message, "error").then(()=> closeModal('modal-withdraw')); 
            }
        }
        ...
        async function loadMyWallet() {
            try {
                ...
                if(activeHtml === "") activeHtml = "<div class='text-center py-6 opacity-50 border border-dashed rounded-xl border-white/20'>ไม่มีรายการฝาก</div>";
                if(historyHtml === "") historyHtml = "<div class='text-center py-2 opacity-30 text-xs'>ไม่มีประวัติการเบิก</div>";
...
export default {
  async fetch(request, env, ctx) {
    ...
    return new Response("API Ready v19", { headers: corsHeaders });
  }
};
"""
...
class AppGenerator(ctk.CTk):
    def __init__(self):
        super().__init__()
        self.title("Nightlife System Builder V19 (Final Cut)")
        self.geometry("1100x800")
        self.vars = { 'bg_color': '#0f172a', 'grad_start': '#1e3a8a', 'grad_end': '#172554', 'text_main': '#ffffff', 'text_sub': '#94a3b8', 'accent': '#EAB308' }
    ...
    def setup_ui(self):
        ...
        ctk.CTkLabel(self.scroll, text="2. Settings", font=("Prompt", 16, "bold"), text_color="#EAB308").pack(anchor="w", padx=20, pady=(20,5))
        self.entries = {}
        for k, h in [("Shop Name", "ชื่อร้าน"), ("LIFF ID", "165xxxx-xxxx"), ("Worker URL", "Cloudflare Worker"), ("Manager Password", "รหัสผ่าน"), ("R2 Public URL", "R2 Domain (Optional)")]:
        ...
        ctk.CTkLabel(self.preview, text="Preview V19", text_color="#555").pack(pady=20)
        self.phone = ctk.CTkFrame(self.preview, width=375, height=667, corner_radius=30, fg_color="#fff", border_width=8, border_color="#333"); self.phone.place(relx=0.5, rely=0.5, anchor="center"); self.phone.pack_propagate(False)
    ...
    def build(self):
        ...
        html = HTML_RAW.replace("__SHOP__", shop).replace("__BG_COLOR__", self.vars['bg_color']).replace("__GRAD_START__", self.vars['grad_start']).replace("__GRAD_END__", self.vars['grad_end']).replace("__TEXT_MAIN__", self.vars['text_main']).replace("__TEXT_SUB__", self.vars['text_sub']).replace("__ACCENT__", self.vars['accent']).replace("__LIFF__", self.entries["LIFF ID"].get()).replace("__WORKER__", w_url).replace("__R2_URL__", self.entries["R2 Public URL"].get())
        worker_code = WORKER_RAW.replace("__MGR_PASS__", mgr)
